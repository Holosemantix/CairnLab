...
- structured passing verifier certificates before verification;
- machine-addressable evidence and structured passing verifier certificates before release;
...
not mutate state. Persistence is a separate engine/store concern.

Transition explanations are rendered outside this module from the returned
`TransitionDecision`. The authority module remains the only source of allowed
or blocked transition semantics.

Verifier certificates may be imported from host systems or generated by
`src/cairnlab/verifiers.py`. In both cases, this module consumes the certificate
as structured evidence and remains the only transition gate.

Reviewer verdicts, reviewer consensus, and naked metadata such as
`metadata.verdict: pass` are not verifier certificates. A certificate must
validate as `VerifierCertificate`, be connected by `verified_by`, target the
requested claim, have `status: pass`, authorize verification, and reference
non-invalidated inputs.
...
`state: verified` or `status: released` is recorded as upstream
`observed_state`; it is not treated as CairnLab authority by itself.
...
The local project facade can persist allowed decisions:
...
Blocked decisions are not persisted unless a caller explicitly records them for
audit with `record_blocked=True`.
...
`force=True` is treated as an explicit human override only when
...
The override can cover release from a non-verified authority state or unresolved
material dissent. It does not waive structural requirements such as human gate,
accountable party, risk assessment, verifier certificate, or high-impact
decision trace package. The proposed transition event records the actor,
authority, liability scope, and rationale.
...
- naked `metadata.verdict: pass` does not authorize verification;
- wrong-claim, missing-input, and invalidated-input certificates block
  verification;
- high-impact release blocks without risk, accountability, and decision trace;
- allowed `--apply` transitions append events and update authority projection;
- release succeeds with required governance;
- unresolved material dissent blocks release unless explicitly overridden by an
  actor with override authority.
