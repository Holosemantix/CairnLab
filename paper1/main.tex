...
\]
only as an orientation for mechanism audits, not as an angle certificate.
...
\subsection{Full-sweep diagnostic dynamics and held-out validation}\label{sec:exp-full-sweep-diagnostics}

Endpoint diagnostics are supplemented by a training-free full-sweep audit over all $4\times3\times9=108$ task--training-seed--checkpoint rows. \Cref{fig:full-sweep-diagnostics} overlays the closed-loop observation-noise score with normalized ATR tail, SMPR failure ($1-\mathrm{SMPR}$), and retained fixed-pool top-1 disagreement. The recovered portions of the Gaussian sweep consistently occupy lower normalized ATR and higher SMPR than fragile rows. We also recompute the fixed 65-candidate pool from checkpoints for all 108 rows, keeping sample-level maximum cost drift and observed top-1 flips. In \Cref{tab:sample-level-certificate-full-sweep}, recovered rows have higher sufficient-event pass rates and much lower fixed-pool top-1 flip rates, while strict q10/q95 gaps remain negative; \Cref{tab:sample-level-event-rate-ci} adds Wilson intervals for the sampled event rates. This strengthens the mechanism audit without turning it into a calibrated probability certificate.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig_full_sweep_diagnostics.png}
\caption{Full-sweep diagnostic dynamics across the Gaussian training grid. Each task panel averages training seeds 3072/3073/3074 at the same \stdmax{} value. Green bands mark behavioral recovery-band points; blue bands mark positive retained fixed-pool proxy gaps. The right-axis curves show normalized ATR tail and SMPR failure ($1-\mathrm{SMPR}$), plus fixed-pool top-1 disagreement; lower is better for all three diagnostic-failure curves.}
\label{fig:full-sweep-diagnostics}
\end{figure}

We also freeze simple diagnostic gates on calibration units before evaluating held-out units. In leave-one-training-seed-out validation, gates calibrated on two seeds and evaluated on the held-out seed have mean absolute recovery-onset error $0.007$ in \stdmax{} units, maximum error $0.020$, precision $0.90$, and recall $0.91$. In leave-one-task-out validation, the corresponding values are $0.008$, $0.020$, $0.90$, and $0.88$. These held-out audits support ATR/SMPR as matched-Gaussian recovered-vs-fragile diagnostics, not as universal checkpoint rankers; task-dependent boundary errors remain visible.

\input{tables/table_heldout_diagnostic_validation}

Threshold sensitivity is reported in Appendix~\ref{sec:appendix-remediation-audits}. Across recovery fractions $0.70/0.80/0.90$ and clean-score tolerances of $3/5/10$ points, all four tasks keep the expected recovered-vs-fragile direction: recovered rows have lower median normalized ATR and higher median SMPR than fragile rows. \Cref{tab:joint-guard-side-validation} keeps the interpretation joint: ATR is the radius term, while SMPR and fixed-pool top-1 flip are guard-side checks and are not standalone robustness metrics. ATR q80/q95 and positive-margin SMPR variants remain unavailable in the retained full-sweep ATR/SMPR artifact rather than inferred.
...
\]
A positive proxy gap is the available recorded-summary counterpart of the fixed-pool margin condition. The retained fixed-pool summaries store median clean candidate margin and q90 paired cost drift; they do not store the lower-tail margin and upper-tail drift summaries needed to instantiate a calibrated q10/q95 certificate. We therefore call this gap a mechanism proxy, not a planner-margin certificate. Thus this audit instantiates the theorem only at the proxy level supported by the recorded summaries: $K$ is observed from the fixed candidate pool, while the single-candidate tail and failure probabilities are not calibrated. ATR-q90, the cost-margin proxy, and SMPR are empirical counterparts of the radius, planning-margin, and discriminability terms. Because the Gaussian sweep uses equal $\stdmax{}$ increments while the tasks have unequal visual and dynamical sensitivity, aggregate grid-point F1 or interval IoU is not used as the primary validation criterion. Appendix~\ref{sec:appendix-radius-margin-params} instead reports boundary-aware recovery/proxy alignment and the fixed-pool top-1 agreement audit used to interpret this diagnostic. The new sample-level full-sweep recomputation checks the sufficient event over the fixed 65-candidate pool for all 108 rows: across all tasks, median cert-pass rises from $0.06$ on fragile rows to $0.61$ on recovered rows, and median top-1 flip falls from $0.54$ to $0.04$. The strict q10/q95 gaps remain negative, so this strengthens the fixed-pool mechanism audit without turning it into a calibrated certificate.


\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig_radius_margin_overlap.png}
\caption{Fixed-pool radius--margin overlap proxy across the full Gaussian training sweep. Curves use the retained fixed-pool summaries: q50 clean candidate margin and twice the q90 paired cost drift. Positive separation supports the radius--margin mechanism at the recorded-summary level. A numerical q10/q95 planner-margin certificate would require recomputing lower-tail margin and upper-tail drift summaries from raw fixed-pool candidate traces or checkpoints, and the sampled pool is fixed rather than adaptive CEM.}
\label{fig:radius-margin-overlap}
\end{figure}

The retained fixed-pool overlap proxy aligns with the broad recovery band but does not exactly reproduce it. TwoRoom and PushT cross the proxy gap within one grid step of the recovery-band onset, Cube aligns on the discrete grid, and Reacher is conservative by two grid steps. This is the expected level of evidence for a diagnostic certificate: it explains the radius--margin mechanism behind the observed recovery band without replacing closed-loop evaluation. The Reacher mismatch is informative rather than hidden: fixed-pool top-1 agreement already rises at the behavioral recovery onset, while the retained-summary cost-margin proxy becomes positive later. To keep the fixed-pool cost-margin proxy separate from a checkpoint-selection claim, we do not turn this proxy into a radius--margin ATR/SMPR gate table. The full-sweep ATR/SMPR held-out validation in \Cref{sec:exp-full-sweep-diagnostics} handles recovered-vs-fragile diagnostic separation; this subsection isolates the retained fixed-pool cost-margin mechanism and its limits.

\subsection{Gaussian sensitivity audits}\label{sec:exp-gaussian-sensitivity}

To check the local Gaussian sensitivity interpretation without claiming a global guarantee, we run two checkpoint-local audits on the base, recovery-onset, and endpoint checkpoints. First, a finite-difference audit perturbs the same observation histories with small Gaussian noise levels and estimates $\mathbb E[R_\sigma^2]/\sigma^2$ from the action-conditioned rollout radius. \Cref{tab:gaussian-sensitivity-audit} reports a consistent endpoint/base reduction in this local slope, from $0.002$ to $0.008$ of the no-noise-checkpoint value across tasks, alongside the ATR contraction. Second, \Cref{tab:jvp-hutchinson-sensitivity-audit} estimates exact-autograd JVP/Hutchinson Frobenius traces for the encoder map, the rollout map, and their composition. The composed trace falls sharply at the endpoint on all tasks (endpoint/base ratios $0.003$, $0.010$, $0.006$, and $0.028$ for TwoRoom, PushT, Reacher, and Cube). The decomposition attributes this mainly to encoder-side sensitivity reduction; the rollout-side trace is not uniformly smaller, so we do not claim a standalone predictor-Jacobian repair. These audits support the composed encoder--predictor sensitivity explanation while remaining local mechanism checks rather than closed-loop robustness proofs.

\subsection{Bounded unseen-stressor check}\label{sec:exp-unseen-scope}

The main evidence remains the matched Gaussian-noise endpoint. After freezing the Gaussian grid, we ran strongest-severity unseen-stressor score checks to test whether the fixed \stdmax{} $=0.08$ endpoint transfers outside the training perturbation family. \Cref{tab:unseen-scope-main} reports the non-Gaussian behavior together with stressor-specific ATR/SMPR diagnostics recomputed under the same blur or resize perturbation. TwoRoom and Reacher improve under strongest blur, whereas PushT and Cube do not show a clear resize gain at the scale of training-seed and evaluation variance. We treat this as bounded behavior outside the matched Gaussian setting.

\begin{table}[H]
\centering
\caption{Bounded unseen-stressor score check with stressor-specific ATR/SMPR diagnostics. Score values are mean $\pm$ population std across LeWM training seeds 3072/3073/3074; each seed point averages evaluation seeds 42/43/44 from strongest-severity unseen runs. The ATR and SMPR columns report raw no-noise $\to$ noise-trained diagnostic values under the row stressor; lower ATR and higher SMPR are better.}
...
This is a controlled diagnostic study, not a method paper. Closed-loop evaluation establishes the behavior; ATR checks whether same-state noisy views have small action-conditioned predictive tails; SMPR checks whether task-grounded near-boundary distinctions survive that contraction. Full-sweep and held-out audits support recovered-vs-fragile diagnostic separation under the matched Gaussian stressor. The radius--margin audit adds fixed-pool cost-margin and sample-level event-rate evidence over the Gaussian grid, and the finite-difference and exact-JVP/Hutchinson audits check the local Gaussian sensitivity mechanism; mismatches are reported as diagnostic limits rather than hidden by the endpoint result. The primary statistic is the full three-training-seed Gaussian replication, with evaluation-seed variance kept as the within-seed measurement scale. These standard deviations are descriptive over three training seeds, not asymptotic significance claims.

\paragraph{Scope.}
Gaussian pixel noise remains the training/evaluation axis. The blur/resize rows are bounded behavior outside the matched Gaussian setting, not a transfer theorem. The radius--margin certificate is fixed-pool and matched-perturbation only: it does not cover adaptive CEM resampling, repeated replanning, or environment-feedback trajectory guarantees. The Gaussian sensitivity audits are local: finite-difference slopes and exact-JVP/Hutchinson trace estimates do not provide a global robustness or closed-loop guarantee. The empirical fixed-stressor diagnostic region is an audit view over matched checkpoints rather than a closed-loop guarantee. ATR/SMPR are not a checkpoint-ranker for \stdmax{} and do not replace closed-loop evaluation inside a broad recovery band; SMPR is only a guard-side component of the joint diagnostic. We do not claim superiority over DrQ-style augmentation, TD-MPC2, DreamerV3, or robust MPC methods; the contribution is a no-retraining diagnostic for JEPA latent world-model checkpoints under a controlled Gaussian stressor.

\paragraph{What remains.}
The results argue against a too-strong reading of latent prediction: future-latent prediction alone does not guarantee closed-loop visual robustness. The joint guard-side audit checks that the low-ATR region also preserves SMPR and fixed-pool candidate stability, but stronger semantic discriminability evidence would still require hand-labeled or simulator-derived contact, topology, action-value, or cost-to-go labels, especially for PushT and Cube. Future methods can turn ATR/SMPR into objectives, but this paper keeps that as a separate method-paper path after diagnostic validity is established.

%% ===========================================================================
\section{Conclusion}\label{sec:conc}

This paper frames Gaussian-noise robustness in JEPA latent world-model control as a selective diagnostic problem. Closed-loop behavior establishes the phenomenon; ATR measures same-state predictive-stability tails; SMPR checks task-grounded anti-collapse margins. On fixed LeWM checkpoints, input-side noise training recovers broad matched-Gaussian recovery bands across three training seeds, and ATR/SMPR move in the expected direction at recovered endpoints.

The theory is deliberately diagnostic: sampled-pool ACPC exposes why tail risk matters and why clean margins prevent closed-loop guarantees, while local Gaussian linearization connects the measured tail to action-conditioned encoder--predictor sensitivity. The resulting view is a radius--margin diagnostic theory for fixed-checkpoint Gaussian robustness: ATR estimates the perturbation-tube radius tail, SMPR estimates margin preservation, and their conjunction defines the empirical fixed-stressor diagnostic region studied here. The broader implication is not solved generalization; it is a concrete audit target for future methods: stable action-conditioned predictive dynamics under nuisance variation, while preserving distinctions needed for planning.
...
\Cref{thm:radius-margin-certificate} separates the quantities needed for a numerical fixed-pool certificate from the lower-sample diagnostic summaries reported in the experiments. In the fixed-pool diagnostic summaries used for \Cref{fig:full-sweep-diagnostics,fig:radius-margin-overlap}, the candidate count is $K=65$ for every task--checkpoint row. The theorem's single-candidate tail probability $\alpha$ is not fixed by the q90 reporting level used by ATR. A q90 summary descriptively leaves 10\% of recorded same-state radii above the reported quantile, but that descriptive exceedance rate is not the $\alpha$ used in the theorem. If it were naively inserted into the union-bound term, the result would be $65\times0.1=6.5$, which is vacuous. A non-vacuous numerical candidate-flip bound would require a much smaller single-candidate tail probability or a different concentration argument. We therefore use q90 ATR and the retained q50/q90 cost-margin overlap as diagnostic evidence for the radius--margin mechanism, not as a calibrated probability guarantee.

\begin{table}[H]
\centering
\caption{Fixed-pool radius--margin interpretation used in the diagnostic validation. The behavioral recovery band is defined from closed-loop observation-only $\sigma=0.08$ scores; the proxy-positive range uses $\widehat Q_{0.50}(\Delta_{\mathcal A})-2\widehat Q_{0.90}(|C_h-C_{\tilde h}|)>0$ on the recorded fixed-pool summaries. These ranges explain the recovery/proxy bands in \Cref{fig:full-sweep-diagnostics} and the overlap proxy in \Cref{fig:radius-margin-overlap}; they are not used to claim a calibrated flip-probability certificate.}
...
\begin{table}[H]
\centering
\caption{Boundary-aware interpretation of the fixed-pool radius--margin proxy. Equal spacing in \stdmax{} is not equal spacing in task sensitivity, so these rows are interpreted as task-conditioned boundary checks rather than grid-point classifier scores.}
...
The recorded fixed-pool flip rate is the clean/noisy top-1 disagreement rate under the same candidate set, so one minus this flip rate gives an empirical top-1 agreement audit that is closer to the theorem's intermediate statement than closed-loop score. It still does not cover adaptive CEM resampling or repeated replanning.

\begin{table}[H]
\centering
\caption{Fixed-pool top-1 agreement audit derived from the recorded fixed-pool flip rate. Rows show the no-noise baseline, the first closed-loop recovery-band checkpoint on the discrete sweep, and the matched \stdmax{} $=0.08$ endpoint.}
...
In this interpretation, $\beta_{\mathrm{plan}}$ and $\beta_{\mathrm{disc}}$ name empirical failure components rather than calibrated probabilities. The planning component is represented by the fixed-pool cost-margin proxy, top-1 agreement audit, and the sample-level event-rate recomputation; the discriminability component is represented by SMPR, equivalently its failure rate $1-\mathrm{SMPR}$. The retained-summary table reports q90 cost drift and q50 clean margin only, while \Cref{tab:sample-level-certificate-full-sweep} separately reports sample-level q10/q95 gaps and event rates. Because those strict q10/q95 gaps remain negative, we do not manufacture certificate-strength tail claims from the recomputation. The closed-loop recovery band explains the fixed-pool radius--margin validation rather than converting the separate ATR/SMPR diagnostic-region audit into a threshold-classifier claim.

\subsection{Remediation audit tables}\label{sec:appendix-remediation-audits}

The held-out validation in \Cref{tab:heldout-diagnostic-validation} is supported by fixed-pool, sample-level, sensitivity, and threshold-sensitivity audits. \Cref{tab:fixed-pool-tail-audit} reports full-sweep top-1 agreement and proxy-gap rows selected from the retained fixed-pool summaries. \Cref{tab:sample-level-certificate-full-sweep} adds the full-sweep sample-level event-rate recomputation, and \Cref{tab:sample-level-event-rate-ci} adds Wilson intervals for the sample-level event rates; these intervals quantify estimation uncertainty, not calibrated theorem probabilities. \Cref{tab:sample-level-certificate-endpoint} preserves the base/endpoint view. \Cref{tab:joint-guard-side-validation} reports ATR together with SMPR and fixed-pool top-1 flip to keep the guard-side interpretation joint rather than standalone. \Cref{tab:gaussian-sensitivity-audit} reports the finite-difference local Gaussian sensitivity audit, and \Cref{tab:jvp-hutchinson-sensitivity-audit} reports the exact-JVP/Hutchinson local trace decomposition. \Cref{tab:threshold-quantile-sensitivity} checks that recovered-vs-fragile ATR/SMPR direction is stable under reasonable behavioral-label thresholds; unavailable ATR/SMPR tail variants are recorded in the audit outputs rather than inferred.
