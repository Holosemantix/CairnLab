...
In this interpretation, $\beta_{\mathrm{plan}}$ and $\beta_{\mathrm{disc}}$ name empirical failure components rather than calibrated probabilities. The planning component is represented by the available fixed-pool cost-margin proxy, and the discriminability component is represented by SMPR, equivalently its failure rate $1-\mathrm{SMPR}$. The table intentionally reports only the cost-margin proxy interval and the closed-loop recovery band, because it explains the fixed-pool radius--margin overlay rather than converting the separate ATR/SMPR diagnostic-region audit into a threshold-classifier claim.
