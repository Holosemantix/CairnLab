...
\paragraph{C3 --- Three-seed validation and release.} We report three main empirical readouts: training-seed robustness, held-out fixed-rule diagnostic triage over the full three-seed grid, and task-semantic margin pass-rate. Appendices keep the larger LeWM/PLDM grids, blur/resize scope checks, ablations, and implementation key maps out of the main narrative.
...
The main text separates three must-read results from reproducibility material. First, \Cref{tab:training-seed-gaussian-lockbox} reports training-seed mean/std for closed-loop Gaussian robustness. Second, \Cref{tab:three-seed-diagnostic-split,tab:three-seed-diagnostic-validation} evaluate one ACPC/PCC/CRA/MAF rule fixed on seed 3072 and then applied to held-out training seeds 3073/3074 over the full 3-seed $\times$ 4-task $\times$ 9-checkpoint LeWM grid. Third, \Cref{tab:semantic-margin-passrate} reports task-semantic margin pass-rates for the no-noise and fixed high-noise endpoints. Unseen-stressor scores, PLDM replication, target-view and heteroscedastic-loss ablations, latent-noise probes, exact key names, and full diagnostic tables are kept as scope checks or reproducibility appendices.
...
Low ACPC would be uninformative if it came from collapsed representations. Because 8-step drift already appears in the ACPC-basin section as rollout-space localization, \Cref{tab:diag-base-vs-repr} keeps only the compact representation checks: effective rank, transition-resolution ratio, and inverse-dynamics probe $R^2$. \Cref{tab:semantic-margin-passrate} then adds the direct semantic margin readout.

\begin{table}[tbp]
\centering
\caption{Compact representation discriminability checks: LeWM-base $\to$ the fixed \stdmax{} $=0.08$ noise-trained checkpoint. These checks are paired with the task-semantic margin pass-rate in \Cref{tab:semantic-margin-passrate}.}
...
The representation checks are paired with a direct task-semantic margin pass-rate. For each task, we choose one semantic-different factor available in the logged state stream: TwoRoom agent position/doorway topology, PushT T-block and pusher pose, Reacher joint/target geometry, and Cube cube-pose/gripper-object geometry. A sample passes if its hard semantic-different clean rollout distance is larger than its same-state clean/noisy rollout radius. \Cref{tab:semantic-margin-passrate} reports the three-training-seed mean/std for the no-noise baseline and the fixed \stdmax{} $=0.08$ endpoint.

\begin{table}[H]
\centering
\caption{Task-semantic selective ACPC study over training seeds 3072/3073/3074. Pass-rate is $P[d_{\mathrm{semantic\ diff}} > d_{\mathrm{same\ state\ noise}}]$ under the same-action rollout readout. The middle columns report median same-state noisy radius and median semantic-different clean rollout distance.}
