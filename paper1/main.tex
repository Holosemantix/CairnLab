...
\paragraph{C3 --- Three-seed validation and release.} We report three main empirical readouts: training-seed robustness, fixed-rule diagnostic triage over the full three-seed grid, and task-semantic margin pass-rate. Appendices keep the larger LeWM/PLDM grids, blur/resize scope checks, ablations, and implementation key maps out of the main narrative.
...
We fix one diagnostic rule before reading the three-seed full-grid outcomes: among nonzero training-noise checkpoints, rank ACPC-$H$/transition lower, PCC lower, CRA higher, and MAF lower, then select the lowest aggregate rank. The rule uses paired rollout/candidate-cost readouts only. It is evaluated on 12 task-seed blocks (4 tasks $\times$ training seeds 3072/3073/3074), each with 8 nonzero checkpoint candidates.
...
The fixed rule is not a replacement for closed-loop evaluation, but it moves ACPC from representative-row explanation toward prospective diagnostic triage. Across the 96 nonzero checkpoint rows, signed ACPC-$H$/transition has Spearman $\rho=0.41$ with observation-noise 0.08 success and $\rho=0.62$ with reduced drop; PCC gives $\rho=0.38$ and $0.60$ on the same two targets. The selection result is strongest on TwoRoom and Reacher, useful but imperfect on PushT, and weakest on Cube, where the semantic/score signal remains the boundary case. We therefore use the fixed rule to prioritize rows and explain plateaus, while closed-loop scores remain the authority for final claims.
...
The table strengthens the interpretation of $R_F$ as a control-facing localization signal: on all four LeWM tasks, smaller paired rollout disagreement is accompanied by lower candidate-cost drift, higher candidate-ranking agreement, and fewer high-margin action flips. Together with \Cref{tab:three-seed-diagnostic-validation}, it shows that rollout/cost readouts provide triage signal beyond encoder radius while still leaving closed-loop evaluation as the final authority.
...
The representation checks are paired with a direct task-semantic margin pass-rate. For each task, we choose one state proxy for semantic-different pairs (TwoRoom position, PushT state, Reacher observation, Cube observation). A sample passes if its hard semantic-different clean rollout distance is larger than its same-state clean/noisy rollout radius. \Cref{tab:semantic-margin-passrate} reports the three-training-seed mean/std for the no-noise baseline and the fixed \stdmax{} $=0.08$ endpoint.

\begin{table}[H]
\centering
\caption{Task-semantic selective ACPC guard over training seeds 3072/3073/3074. Pass-rate is $P[d_{\mathrm{semantic\ diff}} > d_{\mathrm{same\ state\ noise}}]$ under the same-action rollout readout; ratio is median semantic-different distance divided by median same-state noisy radius.}
...
\midrule
TwoRoom & 0.0 & $0.44 \pm 0.06$ & $0.98 \pm 0.04$ & $-0.71 \pm 0.77$ \\
TwoRoom & 0.08 & $1.00 \pm 0.00$ & $29.21 \pm 1.46$ & $15.77 \pm 0.27$ \\
PushT & 0.0 & $0.27 \pm 0.14$ & $0.87 \pm 0.07$ & $-2.48 \pm 1.66$ \\
PushT & 0.08 & $1.00 \pm 0.00$ & $23.66 \pm 2.28$ & $15.06 \pm 0.13$ \\
Reacher & 0.0 & $0.58 \pm 0.03$ & $1.12 \pm 0.03$ & $0.83 \pm 0.37$ \\
Reacher & 0.08 & $1.00 \pm 0.00$ & $61.26 \pm 3.37$ & $16.50 \pm 0.23$ \\
Cube & 0.0 & $0.25 \pm 0.06$ & $0.85 \pm 0.01$ & $-3.32 \pm 0.19$ \\
Cube & 0.08 & $1.00 \pm 0.00$ & $34.44 \pm 4.02$ & $15.99 \pm 0.28$ \\
\bottomrule
\end{tabular}
\end{table}
