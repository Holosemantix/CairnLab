...
- **PLDM base-vs-best ACPC basin replication**: `assets/paper1_data/acpc_basin_diagnostics_pldm_base_best.json` (8 rows: each task's PLDM baseline plus pixels 0.08 point-best checkpoint)
...
- `acpc_basin_diagnostics_pldm_base_best.json` stores a compact PLDM base-vs-pixels-0.08-point-best Gaussian ACPC basin replication. It uses the same same-state clean/noised view protocol as `acpc_basin_diagnostics.json`, but it is not a full 4 tasks × 9 configs PLDM basin sweep.
...
- `tools/check_paper1_consistency.py` verifies:
  ...
  - the PLDM base-vs-best ACPC basin replication covers exactly the four baseline rows and four pixels 0.08 point-best rows, all `ok`
