...
The fixed rule is not a replacement for closed-loop evaluation: it tests whether ACPC-family readouts diagnose entry into a closed-loop robustness plateau. Because the sweep has broad finite-variance plateaus, Appendix~\ref{sec:appendix-selector-audit} treats \stdmax{} as a candidate label inside each task--training-seed block and evaluates membership screens rather than point-best ranking. With a 5pp plateau label, the aggregate top-half screen intersects the closed-loop plateau in 12/12 task--training-seed blocks; 42 of its 48 screened rows are plateau members (87.5\% precision, 61.8\% recall), versus 70.8\% precision for an exact random top-half reference. This supports plateau-entry diagnosis: the readouts identify a candidate region enriched for robust checkpoints and reveal boundary blocks such as Cube seed 3072, while closed-loop scores remain the authority and plateau-internal ordering is not a claim.
...
Random top-half reference & $11.96/12$ & $70.8\%$ & $50.0\%$ & $34.0/14.0/34.0$ & exact reference, not a training baseline \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Reading.} The audit evaluates membership enrichment, not point-optimal ranking. The aggregate screen intersects the closed-loop plateau in every task--training-seed block and has higher precision than an exact random top-half screen, while still leaving false positives and false negatives. This is the intended use: diagnose whether a checkpoint region has entered the robustness plateau, expose boundary blocks, and leave final checkpoint claims to closed-loop evaluation.
