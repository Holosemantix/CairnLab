...
\begin{abstract}
Latent predictive world models such as JEPAs avoid pixel reconstruction, but latent prediction alone does not define robustness for closed-loop control. We introduce action-conditioned predictive consistency (ACPC) as a diagnostic for this gap: clean and corrupted views of the same state should induce similar rollout readouts under the same action sequence, while action-relevant state differences remain separable. We connect ACPC to candidate-cost drift and top-1 stability on fixed and sampled candidate pools, with explicit ACPC-tail and clean-margin terms, and show that the Gaussian basin radius measures action-conditioned encoder--predictor sensitivity rather than encoder invariance alone. In a controlled Gaussian-noise study on PushT, TwoRoom, Reacher, and Cube, no-noise LeWM is fragile under observation-only noise, e.g., PushT falls from $86.33\%$ to $4.33\%$ and Reacher from $58.67\%$ to $18.33\%$. Full-sequence input-side noise training supplies recovered checkpoints for diagnosis, and the closed-loop pattern replicates across three training seeds (3072/3073/3074). A ranking rule fixed on the seed-3072 development grid selects checkpoints within 5 points of the closed-loop best in 7/8 held-out task-seed blocks (10/12 over all three seeds), with held-out mean regret $2.21\pm1.83$ points; it is a triage signal, not a substitute for closed-loop evaluation. A task-state proxy margin pass-rate further checks selective consistency, rising from $0.25$--$0.58$ for no-noise baselines to approximately $1.00$ at the fixed $\stdmax=0.08$ endpoint. The result is a no-retraining diagnostic framework for matched Gaussian-noise stress tests, with appendix ablations and bounded unseen-stressor checks documenting scope and failure modes.
...
\paragraph{C3 --- Three-seed validation and release.} We report three main empirical readouts: training-seed robustness, held-out fixed-rule diagnostic triage over the full three-seed grid, and task-state proxy margin pass-rate. Appendices keep the larger LeWM/PLDM grids, blur/resize scope checks, ablations, and implementation key maps out of the main narrative.
...
Was contraction selective? & task-state proxy margin plus transition/ID proxy checks & checks that action-relevant distinctions are not erased while nuisance views contract \\
\bottomrule
\end{tabularx}
\end{table}
...
The main text separates three must-read results from reproducibility material. First, \Cref{tab:training-seed-gaussian-lockbox} reports training-seed mean/std for closed-loop Gaussian robustness. Second, \Cref{tab:three-seed-diagnostic-split,tab:three-seed-diagnostic-validation} evaluate one ACPC/PCC/CRA/MAF rule fixed on seed 3072 and then applied to held-out training seeds 3073/3074 over the full 3-seed $\times$ 4-task $\times$ 9-checkpoint LeWM grid. Third, \Cref{tab:semantic-margin-passrate} reports task-state proxy margin pass-rates for the no-noise and fixed high-noise endpoints. Unseen-stressor scores, PLDM replication, target-view and heteroscedastic-loss ablations, latent-noise probes, exact key names, and full diagnostic tables are kept as scope checks or reproducibility appendices.
...
The fixed rule is not a replacement for closed-loop evaluation, but it moves ACPC from representative-row explanation toward held-out diagnostic triage. Across the 96 nonzero checkpoint rows, signed ACPC-$H$/transition has Spearman $\rho=0.41$ with observation-noise 0.08 success and $\rho=0.62$ with reduced drop; PCC gives $\rho=0.38$ and $0.60$ on the same two targets. The held-out triage result is strongest on TwoRoom and Reacher, useful but imperfect on PushT, and weakest on Cube. The clearest miss is Cube seed 3072: the fixed rule chooses $\stdmax{}=0.05$ with $60.00\%$ observation-noise 0.08 success, while the closed-loop best is $\stdmax{}=0.03$ at $68.33\%$ (regret $8.33$ pp). PushT seed 3073 is the other beyond-5pp miss ($81.00\%$ selected at $\stdmax{}=0.08$ versus $86.67\%$ best at $\stdmax{}=0.04$). A selector-baseline audit in Appendix~\ref{sec:appendix-selector-audit} shows the same boundary: the aggregate rule is comparable to fixed $\stdmax{}=0.08$ and single-metric selectors on broad Gaussian plateaus, but better than exact random nonzero-std selection. We therefore use the fixed rule to prioritize rows and explain plateaus, while closed-loop scores remain the authority for final claims.
...
This comparison localizes where the intervention changed same-state perturbation behavior. The compact rows summarize the high-noise regime visible in closed-loop evaluation; the fixed-rule validation in \Cref{tab:three-seed-diagnostic-validation} handles checkpoint triage across the full three-seed grid. Appendix~\ref{sec:appendix-acpc-basin-grid} reports the full basin grid, and \Cref{sec:exp-downstream-readouts} reports a compact shared-candidate downstream check.
...
The table strengthens the interpretation of $R_F$ as a control-facing localization signal: on all four LeWM tasks, smaller paired rollout disagreement is accompanied by lower candidate-cost drift, higher candidate-ranking agreement, and fewer high-margin action flips. This is the empirical counterpart of \Cref{thm:sampled-pool-acpc}: ACPC-$H$ and PCC estimate the drift/tail side of the bound, CRA checks whether sampled candidate rankings remain aligned, and MAF estimates the high-margin flip failure term after margin filtering. The fixed-rule validation in \Cref{tab:three-seed-diagnostic-split,tab:three-seed-diagnostic-validation} uses the same theory-matched readout family for no-retraining triage, while closed-loop evaluation remains the final authority.

\subsection{Selective consistency and failure checks}\label{sec:exp-diag}

Low ACPC would be uninformative if it came from collapsed representations. Because 8-step drift already appears in the ACPC-basin section as rollout-space localization, \Cref{tab:diag-base-vs-repr} keeps only the compact representation checks: effective rank, transition-resolution ratio, and inverse-dynamics probe $R^2$. \Cref{tab:semantic-margin-passrate} then adds the task-state proxy margin readout.

\begin{table}[tbp]
\centering
\caption{Compact representation discriminability checks: LeWM-base $\to$ the fixed \stdmax{} $=0.08$ noise-trained checkpoint. These checks are paired with the task-state proxy margin pass-rate in \Cref{tab:semantic-margin-passrate}.}
...
The representation checks are paired with a task-state proxy margin pass-rate. For each task, we choose one state-proxy factor available in the logged stream: TwoRoom agent position/doorway topology, PushT T-block and pusher pose, Reacher joint/target geometry, and Cube cube-pose/gripper-object geometry. A sample passes if its hard state-proxy-different clean rollout distance is larger than its same-state clean/noisy rollout radius. The pairs are selected by a fixed median-quantile state-distance rule, so this is not an oracle near-boundary semantic proof; contact-specific keypoints and hand-labeled near-boundary pairs remain future validation. \Cref{tab:semantic-margin-passrate} reports the three-training-seed mean/std for the no-noise baseline and the fixed \stdmax{} $=0.08$ endpoint.

\begin{table}[H]
\centering
\caption{Task-state proxy selective ACPC study over training seeds 3072/3073/3074. Pass-rate is $P[d_{\mathrm{proxy\ diff}} > d_{\mathrm{same\ state\ noise}}]$ under the same-action rollout readout. The middle columns report median same-state noisy radius and median state-proxy-different clean rollout distance.}
\label{tab:semantic-margin-passrate}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{llrrrr}
\toprule
Task & std & pass-rate & same radius & proxy diff & margin \\
\midrule
TwoRoom & 0.0 & $0.44 \pm 0.06$ & $17.43 \pm 0.65$ & $17.06 \pm 0.11$ & $-0.71 \pm 0.77$ \\
TwoRoom & 0.08 & $1.00 \pm 0.00$ & $0.56 \pm 0.02$ & $16.40 \pm 0.25$ & $15.77 \pm 0.27$ \\
PushT & 0.0 & $0.27 \pm 0.14$ & $18.35 \pm 1.21$ & $15.82 \pm 0.20$ & $-2.48 \pm 1.66$ \\
PushT & 0.08 & $1.00 \pm 0.00$ & $0.69 \pm 0.06$ & $16.24 \pm 0.02$ & $15.06 \pm 0.13$ \\
Reacher & 0.0 & $0.58 \pm 0.03$ & $15.44 \pm 0.51$ & $17.24 \pm 0.13$ & $0.83 \pm 0.37$ \\
Reacher & 0.08 & $1.00 \pm 0.00$ & $0.27 \pm 0.02$ & $16.76 \pm 0.21$ & $16.50 \pm 0.23$ \\
Cube & 0.0 & $0.25 \pm 0.06$ & $19.33 \pm 0.15$ & $16.46 \pm 0.13$ & $-3.32 \pm 0.19$ \\
Cube & 0.08 & $1.00 \pm 0.00$ & $0.49 \pm 0.05$ & $16.59 \pm 0.24$ & $15.99 \pm 0.28$ \\
\bottomrule
\end{tabular}
\end{table}

The task-state proxy margin supplies the missing selective half of ACPC more directly than rank or ID probes alone, while staying below an oracle semantic-label claim. The high-noise endpoint keeps state-proxy-different rollouts separated while shrinking same-state noisy rollouts on all four tasks; the no-noise baselines often fail this margin, especially PushT and Cube. Each task/endpoint aggregates 100 sampled pairs per training seed and three training seeds. Treating these 300 pairs as independent only for calibration, Hoeffding gives $p \ge \hat p-\sqrt{\log(1/\alpha)/(2n)}$; for $\hat p=1.00$, $n=300$, and $\alpha=0.05$, the lower confidence bound is about $0.929$. Nearby dataset windows may be dependent, so we use this as a finite-sample calibration rather than a dataset-wide formal guarantee. The heteroscedastic-loss ablation in Appendix~\ref{sec:appendix-D} remains the failure case that motivates the guard: PushT transition L2 falls from $0.3015$ to $0.1023$, the ID probe falls from $0.7739$ to $0.2678$, and unperturbed success collapses to $13.33\%$. The longer representation, rollout-drift, latent-noise, and task-by-task mechanism diagnostics remain in Appendix~\ref{sec:appendix-diagnostic-framework}--\ref{sec:appendix-mech-attribution}.
...
\paragraph{Hoeffding calibration for the task-state proxy margin.}
...
%% ===========================================================================
\section{Selector-baseline audit for fixed-rule triage}\label{sec:appendix-selector-audit}
...
\begin{table}[H]
\centering
\caption{Selector-baseline audit for no-retraining checkpoint triage. Regret is closed-loop best observation-noise 0.08 success minus selected success. The aggregate rule is comparable to fixed $\stdmax{}=0.08$ and single-metric selectors on the broad Gaussian plateaus; the audit is therefore plateau context rather than evidence that the aggregate rule dominates all obvious baselines.}
...
\midrule
Aggregate ACPC/PCC/CRA/MAF & all & $10/12$ & $2.25 \pm 2.51$ & triage/plateau localization \\
...
\midrule
Aggregate ACPC/PCC/CRA/MAF & held-out 3073/3074 & $7/8$ & $2.21 \pm 1.83$ & held-out triage signal \\
