...
Endpoint diagnostics are supplemented by a training-free full-sweep audit over all $4\times3\times9=108$ task--training-seed--checkpoint rows. \Cref{fig:full-sweep-diagnostics} overlays the closed-loop observation-noise score with normalized ATR tail, SMPR failure ($1-\mathrm{SMPR}$), and retained fixed-pool top-1 disagreement. The recovered portions of the Gaussian sweep consistently occupy lower normalized ATR and higher SMPR than fragile rows. The retained fixed-pool quantities move in the same broad direction, but the full-sweep rows remain summary-level audits: sample-level maximum cost-drift traces are not retained for all 108 rows, so full-sweep pool-level sufficient-event pass rates are not inferred.
...
Threshold sensitivity is reported in Appendix~\ref{sec:appendix-remediation-audits}. Across recovery fractions $0.70/0.80/0.90$ and clean-score tolerances of $3/5/10$ points, all four tasks keep the expected recovered-vs-fragile direction: recovered rows have lower median normalized ATR and higher median SMPR than fragile rows. Other requested tail variants, including ATR q80/q95, SMPR positive margins, and q10/q95 cost-margin gaps, require raw sample-level tails that are not retained in the current summaries.
...
In this interpretation, $\beta_{\mathrm{plan}}$ and $\beta_{\mathrm{disc}}$ name empirical failure components rather than calibrated probabilities. The planning component is represented by the available fixed-pool cost-margin proxy and top-1 agreement audit, and the discriminability component is represented by SMPR, equivalently its failure rate $1-\mathrm{SMPR}$. The table intentionally reports only the retained q90 cost drift and q50 clean margin because the diagnostic summaries do not include lower-tail clean-margin and upper-tail paired-drift fields for a q10/q95 numerical certificate, nor sample-level radii for q95/q99 sensitivity; we therefore do not manufacture certificate-strength tail claims from unavailable raw tails. The closed-loop recovery band explains the fixed-pool radius--margin validation rather than converting the separate ATR/SMPR diagnostic-region audit into a threshold-classifier claim.

\subsection{Remediation audit tables}\label{sec:appendix-remediation-audits}

The held-out validation in \Cref{tab:heldout-diagnostic-validation} is supported by fixed-pool and threshold-sensitivity audits. \Cref{tab:fixed-pool-tail-audit} reports full-sweep top-1 agreement and proxy-gap rows selected from the retained fixed-pool summaries. \Cref{tab:sample-level-certificate-endpoint} adds an endpoint-only recomputation from checkpoints that keeps sample-level maximum cost drift over the fixed candidate pool. This audit supports the mechanism through cert-pass and flip-rate separation, while its strict q10/q95 gaps remain negative. \Cref{tab:threshold-quantile-sensitivity} checks that recovered-vs-fragile ATR/SMPR direction is stable under reasonable behavioral-label thresholds; unavailable full-sweep tail variants are recorded in the audit outputs rather than inferred.
