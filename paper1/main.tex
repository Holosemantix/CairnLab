...
\subsection{Noise augmentation reveals matched-Gaussian recovery bands}\label{sec:exp-sweep}
...
\subsection{Full-sweep diagnostic separation and held-out validation}\label{sec:exp-full-sweep-diagnostics}

Endpoint diagnostics are supplemented by a training-free full-sweep analysis over all $4\times3\times9=108$ task--training-seed--checkpoint rows. \Cref{fig:full-sweep-diagnostics} overlays the closed-loop observation-noise score with normalized ATR tail, SMPR failure ($1-\mathrm{SMPR}$), and fixed-pool top-1 disagreement. The recovered portions of the Gaussian sweep consistently occupy lower normalized ATR and higher SMPR than fragile rows. We also recompute the fixed 65-candidate pool from checkpoints for all 108 rows, keeping sample-level maximum cost drift and observed top-1 flips. In \Cref{tab:sample-level-certificate-full-sweep}, recovered rows have higher sufficient-event pass rates and much lower fixed-pool top-1 flip rates, while strict q10/q95 gaps remain negative. \Cref{tab:sample-level-event-rate-ci} turns the sample-level recomputation into a finite-sample empirical fixed-pool risk audit by reporting Wilson intervals for cert-pass, top-1 flip, and top-1 flip conditional on cert-pass. This treats q10/q95 as a conservative negative calibration result and uses paired event rates, rather than a tuned quantile gap, as the fixed-pool evidence. The evidence remains a fixed-pool empirical analysis, not a calibrated theorem probability bound.
...
\subsection{Fixed-pool radius--margin calibration}\label{sec:exp-radius-margin}
...
\subsection{Evaluation under bounded non-Gaussian stressors}\label{sec:exp-unseen-scope}
...
\subsection{Finite-sample empirical fixed-pool risk audit}\label{sec:appendix-fixed-pool-risk-calibration}
...
\subsection{Supplementary diagnostic analyses}\label{sec:appendix-supplementary-diagnostics}

The held-out validation in \Cref{tab:heldout-diagnostic-validation} is supported by supplementary fixed-pool, sample-level, sensitivity, and threshold-sensitivity analyses. \Cref{tab:theory-evidence-map} gives a Theory-to-evidence map for the matched-Gaussian diagnostic, summarizing how the theoretical quantities are instantiated empirically and where each audit stops. \Cref{tab:fixed-pool-tail-audit} reports full-sweep top-1 agreement and proxy-gap rows selected from the available summary-level fixed-pool summaries. \Cref{tab:sample-level-certificate-full-sweep} adds the full-sweep sample-level event-rate recomputation, and \Cref{tab:sample-level-event-rate-ci} reports the finite-sample empirical fixed-pool risk audit: cert-pass, top-1 flip, and top-1 flip conditional on cert-pass with Wilson intervals. These intervals quantify estimation uncertainty, not calibrated theorem probabilities. \Cref{tab:sample-level-certificate-endpoint} preserves the base/endpoint view. \Cref{tab:joint-guard-side-validation} reports ATR together with SMPR and fixed-pool top-1 flip to keep the guard-side interpretation joint rather than standalone. \Cref{tab:gaussian-sensitivity-audit} reports the finite-difference local Gaussian sensitivity analysis, and \Cref{tab:jvp-hutchinson-sensitivity-audit} reports the exact-JVP/Hutchinson local trace decomposition. \Cref{tab:threshold-quantile-sensitivity} evaluates whether recovered-vs-fragile ATR/SMPR direction is stable under reasonable behavioral-label thresholds; unavailable ATR/SMPR tail variants are reported as unavailable rather than inferred.
