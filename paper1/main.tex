...
The basin contraction is not treated as sufficient evidence of robustness. It is paired with the next table's rank, transition-resolution, inverse-dynamics, and heteroscedastic-loss checks to rule out the trivial explanation that the model simply collapsed action-relevant state distinctions. A separate shared-candidate sanity check in Appendix~\ref{sec:appendix-phase0} follows the same direction on the primary observation-noise endpoint with an unperturbed goal: for LeWM PushT, ACPC-$H$/transition drops from $2.625$ to $0.146$, PCC from $56.6$ to $2.6$, candidate-ranking agreement rises from $0.412$ to $0.997$, and margin-conditioned action flips drop from $0.98$ to $0.08$. Because this check uses random candidate sets and representative rows known from the closed-loop sweep, we keep it as face-validity evidence rather than as the main robustness claim.
...
\paragraph{Reading.} TwoRoom and Reacher show the desired agreement: the unseen stress scores improve strongly and ACPC-$H$/transition, PCC, CRA, and MAF move in the better direction. Cube is the useful negative boundary: the unseen score does not improve and ACPC/CRA/MAF do not improve. PushT is seed-sensitive: seed 3073 aligns, but seed 3074 loses resize stress performance, so the seed average remains weak even though some local diagnostic terms improve. We therefore treat this as bounded appendix evidence for diagnostic localization under selected unseen stressors, not as evidence that ACPC is a universal cross-perturbation robustness predictor.
