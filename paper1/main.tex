...
\paragraph{C3 --- Fixed-checkpoint evidence.} Across three LeWM training seeds, matched-Gaussian robustness plateaus coincide with large ATR reductions and near-one SMPR at recovered endpoints. Unseen-stressor scores bound the perturbation scope without turning the diagnostic into a method or universal transfer claim.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\linewidth]{fig_atr_smpr_plane.png}
\caption{Compressed selective-ACPC diagnostic plane. Each arrow moves from the no-noise baseline to the fixed \stdmax{} $=0.08$ checkpoint; lower ATR and higher SMPR are the intended diagnostic direction. Error bars show population std across training seeds.}
\label{fig:atr-smpr-plane}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig_feature_neighborhood_atr_smpr.png}
\caption{Qualitative PushT feature-neighborhood illustration for the same no-noise versus \stdmax{} $=0.08$ contrast. Circles mark clean anchors and triangles mark same-state noisy views after a shared 2-D projection of encoder features or post-predictor rollout features. The panel is visual intuition only; ATR and SMPR remain the quantitative paper-facing readouts.}
\label{fig:feature-neighborhood-atr-smpr}
\end{figure}

The two readouts move in the intended selective direction on all four tasks: recovered endpoints have much smaller same-state predictive tails and much higher task-grounded margin pass rates. \Cref{fig:atr-smpr-plane} visualizes the same movement as \Cref{tab:compressed-selective-acpc}; it is not an additional diagnostic or a point-ranking rule inside a plateau. \Cref{fig:feature-neighborhood-atr-smpr} provides a qualitative feature-neighborhood illustration of the same intuition on PushT: same-state noisy views become tighter after noise training, while the quantitative claim still rests on ATR, SMPR, and closed-loop behavior. Cube remains the useful boundary case: the compressed diagnostics improve strongly, but the closed-loop Gaussian gain and regret in \Cref{tab:training-seed-gaussian-lockbox} are weaker than PushT or Reacher, so closed-loop evaluation remains the behavioral authority.
