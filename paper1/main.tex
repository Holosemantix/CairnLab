...
The proxy aligns with the broad recovery interval but does not exactly reproduce it. TwoRoom and PushT cross the proxy gap near the start of their high-score intervals, Cube aligns over the same broad range, and Reacher shows early behavioral recovery before the cost-margin proxy becomes positive. This is the expected level of evidence for a diagnostic certificate: it explains the radius--margin mechanism behind the existing plateau without replacing closed-loop evaluation. Full-sweep SMPR is unavailable in the recorded diagnostic summaries, so we do not report an ATR-only/SMPR-only/joint-gate ablation over the entire grid. Instead, endpoint SMPR in \Cref{tab:atr-smpr-selective-acpc} supplies the task-discriminability side for the no-noise versus recovered endpoint contrast.
...
In this interpretation, $\beta_{\mathrm{plan}}$ is the empirical chance that the clean top-1/top-2 fixed-pool margin is too small relative to the cost drift, and $\beta_{\mathrm{disc}}$ is the chance that task-grounded different-state separations fall inside the perturbation tube. The experiments estimate these sides with available fixed-pool margin summaries and endpoint SMPR, respectively. Full-sweep SMPR is unavailable, so the table reports only the cost-margin proxy interval and the closed-loop behavioral plateau interval.
