...
In this interpretation, $\beta_{\mathrm{plan}}$ is the empirical chance that the clean top-1/top-2 fixed-pool margin is too small relative to the cost drift, and $\beta_{\mathrm{disc}}$ is the chance that task-grounded different-state separations fall inside the perturbation tube. The experiments estimate the planning side with available fixed-pool margin summaries and estimate the discriminability side with SMPR. The table intentionally reports only the cost-margin proxy interval and the closed-loop behavioral plateau interval, because it explains the fixed-pool radius--margin overlay rather than the separate ATR/SMPR threshold audit.
