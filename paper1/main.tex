...
\paragraph{C3 --- Fixed-checkpoint evidence chain.} On fixed checkpoints, we report a three-training-seed evidence chain: matched-Gaussian recovery; a frozen diagnostic plateau-membership screen; selective-margin and planner-side checks; and bounded blur/resize checks. Larger grids, PLDM replication, ablations, and artifact paths remain in the appendix and release manifest.
...
The empirical analysis follows three readout families: closed-loop success/drop for behavioral effect, ACPC rollout/cost diagnostics ($R_F$, PCC/CRA/MAF, margin-conditioned flips, and a reduced-budget CEM trace audit) for action-conditioned predictive stability, and task-grounded near-boundary margins for selective discriminability. Auxiliary ADM/SPRR, latent-noise, artifact-key, and full representation diagnostics remain in the appendices.
...
In the reported basin diagnostic, this condition is instantiated with the L2 rollout-token distance underlying $R_F$; other downstream readouts are empirical companion diagnostics rather than assumptions in the fixed-candidate bound.
...
A three-seed blur/resize score check is reported in Appendix~\ref{sec:appendix-unseen-transfer}. It is an auxiliary severe-stressor check, not part of the three main results. The following validation stays within the frozen Gaussian grid and asks whether ACPC-family readouts identify plateau-member candidate regions across all three LeWM training seeds.

\subsection{Three-seed frozen diagnostic validation}\label{sec:exp-three-seed-diagnostic-validation}

We freeze an aggregate diagnostic score on the seed-3072 development grid before evaluating independent training seeds: among nonzero training-noise checkpoints, lower ACPC-$H$/transition, lower PCC, higher CRA, and lower MAF receive better ranks, and the ranks are summed. We use this frozen score in two audit views. First, \Cref{tab:three-seed-diagnostic-validation} reports a single-row proximity view: the minimum-score row in each task--seed block is judged only by distance to the closed-loop plateau. Second, Appendix~\ref{sec:appendix-selector-audit} gives the main plateau-membership screen, which returns the top half of candidates in each task--seed block. No closed-loop success enters either diagnostic score.
...
\begin{table}[H]
\centering
\caption{Three-seed frozen diagnostic validation on the full LeWM Gaussian grid. Plateau hit counts how often the single-row proximity view is within five percentage points of the closed-loop best observation-noise 0.08 checkpoint in the same task/seed block. Proximity gap is best minus the reported row's observation-noise 0.08 success. Point-best hit and top-2 proximity overlap are context only; the paper-facing criterion is plateau proximity.}
\label{tab:three-seed-diagnostic-validation}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lrrrr}
\toprule
Task & point-best hit & plateau hit & proximity gap & top-2 proximity overlap \\
\midrule
...
This comparison localizes where the intervention changed same-state perturbation behavior. The compact rows summarize the high-noise regime visible in closed-loop evaluation; the frozen diagnostic validation in \Cref{tab:three-seed-diagnostic-validation} audits plateau localization across the full three-seed grid. The full basin grid is reported in the appendix and release manifest, and \Cref{sec:exp-downstream-readouts} reports a compact shared-candidate downstream check.
...
These rows connect the ACPC basin to planner-facing quantities: on all four LeWM tasks, smaller paired rollout disagreement coincides with lower candidate-cost drift, higher candidate-ranking agreement, and fewer high-margin action flips. This is the empirical counterpart of \Cref{thm:sampled-pool-acpc}: ACPC-$H$ and PCC estimate the drift/tail side of the bound, CRA checks whether sampled candidate rankings remain aligned, and MAF estimates the high-margin flip failure term after margin filtering. The frozen diagnostic validation in \Cref{tab:three-seed-diagnostic-validation} uses the same theory-matched readout family for no-retraining plateau localization.
...
The main claim remains matched Gaussian-noise robustness. After freezing the Gaussian grid, we also ran a strongest-severity unseen-stressor check to ask whether the diagnostics reflect behavior outside the training perturbation family. \Cref{tab:unseen-scope-main} shows a split pattern. TwoRoom and Reacher improve under strongest blur, and paired ACPC/PCC/CRA diagnostics move in the same direction. PushT and Cube do not show a clear score gain at the scale of training-seed and evaluation variance, and their diagnostic deltas are weak or mixed. A full appendix audit over all task--family--seed endpoints for blur and resize gives the same directional readout: the composite ACPC/PCC/CRA/MAF rank has Spearman $\rho=0.94$ with stress-success delta over 24 rows. We therefore use this table as bounded diagnostic-specificity evidence, not as a universal cross-perturbation robustness claim.

\begin{table}[H]
\centering
\caption{Bounded unseen-stressor check. Score $\Delta$ is the fixed $\stdmax{}=0.08$ checkpoint minus the no-noise baseline under the strongest unseen stress, averaged over training seeds 3072/3073/3074. Diagnostic deltas are from the matched paired-diagnostic slice averaged over training seeds 3072/3073/3074; negative is better for ACPC-$H$/transition and PCC, positive is better for CRA. The table is auxiliary evidence, not a universal transfer claim.}
...
This paper frames Gaussian-noise robustness in JEPA latent world-model control as a staged diagnostic problem. Encoder geometry shows how visual perturbations enter the latent state; ACPC asks whether they change action-conditioned futures and candidate costs; and a task-grounded near-boundary proxy margin checks whether robustness comes at the cost of erasing coarse action-relevant distinctions. On fixed LeWM checkpoints, input-side noise training recovers broad matched-Gaussian robustness plateaus across three training seeds, and the paired diagnostics localize that recovery through action-conditioned rollout/cost stability, selective margins, and reduced optimizer sensitivity.
...
\paragraph{Reading.} These proofs calibrate the diagnostics used in the main text; they do not provide closed-loop control guarantees.
...
\paragraph{Reading.} Reproducibility details.
...
\paragraph{Reading.} Auxiliary definitions.
...
\paragraph{Reading.} The full LeWM-base diagnostic profile is released in \codebrk{assets/paper1\_data/canonical\_diagnostics\_20260517.json}; reported key names are indexed in \code{DATA\_MANIFEST.md}. The argument uses only the compact discriminability and rollout/cost readouts.


%% ===========================================================================
\section{Heteroscedastic-loss formulation}\label{sec:appendix-C}

\paragraph{Reading.} Ablation loss definition.
...
\paragraph{Reading.} This appendix records the full heteroscedastic-loss data referenced in \Cref{sec:disc}. The $\sigma$-head is trained jointly with the predictor; gradient is detached on the $\sigma$ path so the mean prediction path is exactly LeWM MSE when $\sigma$ is constant.
...
\paragraph{Reading.} Descriptive mechanism check.
...
\begin{table}[p]
\centering
\caption{Full blur/resize unseen-stressor diagnostic audit. Rows are the 24 task--family--training-seed endpoints: four tasks, blur and resize strongest stressors, and training seeds 3072/3073/3074. Correlations compare diagnostic deltas from \stdmax{} $=0.0\to0.08$ against closed-loop strongest-stress score deltas and drop improvements. This table reduces selected-slice risk but remains a bounded two-family audit.}
...
\paragraph{Reading.} This appendix records the released paired-diagnostic artifacts behind the compact main-text readouts; these artifacts are not used for model selection.
...
\paragraph{Reading.} The audit evaluates membership enrichment, not point-optimal ranking. The aggregate screen improves precision over exact random top-half screening (87.5\% vs.\ 70.8\%). Single-readout and high-noise reference rows remain in the released artifact as transparency checks, not paper-facing selector baselines, because the diagnostic screen is not meant to beat training-noise strength or individual readouts as a point selector. We therefore read the screen as diagnostic enrichment and failure localization.
...
\paragraph{Reading.} The ablation acts as a negative auxiliary check and supports retaining the full-sequence empirical branch. Simply changing the prediction target to the original future does not reproduce the recovery from input-side noise augmentation in \Cref{tab:sweep-summary,fig:sweep}: PushT and Reacher remain close to their no-noise robustness cliffs, TwoRoom degrades under stronger eval noise despite high unperturbed success, and Cube does not improve over the main sweep.
