...
\begin{table}[H]
\centering
\caption{Selective ACPC endpoint diagnostics over training seeds 3072/3073/3074. ATR is the q90 normalized same-state clean/noisy action-conditioned rollout disagreement; lower is better. SMPR is the task-grounded near-boundary selective-margin pass rate; higher is better.}
...
\section{Diagnostic Validation and Sensitivity Details}\label{sec:appendix-supplementary-diagnostics}

The held-out validation in \Cref{tab:heldout-diagnostic-validation} is supported by diagnostic-region, guard-side, sensitivity, and threshold-sensitivity analyses. \Cref{tab:theory-evidence-map} maps each theoretical object to its empirical evidence and limitation. \Cref{tab:joint-guard-side-validation} reports ATR together with SMPR and fixed-pool top-1 flip to keep the guard-side interpretation joint rather than standalone. \Cref{tab:gaussian-sensitivity-audit} reports the finite-difference local Gaussian sensitivity analysis, and \Cref{tab:jvp-hutchinson-sensitivity-audit} reports the exact-JVP/Hutchinson local trace decomposition. \Cref{tab:threshold-quantile-sensitivity} evaluates whether recovered-vs-fragile ATR/SMPR direction is stable under reasonable behavioral-label thresholds; unavailable ATR/SMPR tail variants are reported as unavailable rather than inferred.
