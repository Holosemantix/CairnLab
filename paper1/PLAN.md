...
**状态：near submit-ready；ACPC dense Gaussian-noise basin diagnostic、Phase 0 exploratory appendix、target-view negative ablation 已补并已降权，reference audit / PDF / consistency checks 已通过。**
...
- `paper1/main.pdf` 可 clean build（33 pages）；`tools/check_paper1_consistency.py` 通过；`git diff --check` 通过；最终 LaTeX log 无 overfull/underfull、undefined citation/reference、缺图或 fatal error。
