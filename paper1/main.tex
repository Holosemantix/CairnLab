...
Endpoint diagnostics are supplemented by a training-free full-sweep audit over all $4\times3\times9=108$ task--training-seed--checkpoint rows. \Cref{fig:full-sweep-diagnostics} overlays the closed-loop observation-noise score with normalized ATR, SMPR failure ($1-\mathrm{SMPR}$), and retained fixed-pool top-1 disagreement. The recovered portions of the Gaussian sweep consistently occupy lower normalized ATR and higher SMPR than fragile rows. The retained fixed-pool quantities move in the same broad direction, but they remain summary-level audits: sample-level maximum cost-drift traces are not retained, so we do not report pool-level sufficient-event pass rates.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig_full_sweep_diagnostics.png}
\caption{Full-sweep diagnostic dynamics across the Gaussian training grid. Each task panel averages training seeds 3072/3073/3074 at the same \stdmax{} value. Green bands mark behavioral recovery-band points; blue bands mark positive retained fixed-pool proxy gaps. The right-axis curves show normalized ATR, SMPR failure ($1-\mathrm{SMPR}$), and fixed-pool top-1 disagreement; lower is better for all three diagnostic-failure curves.}
...
\]
A positive proxy gap is the available recorded-summary counterpart of the fixed-pool margin condition. The retained fixed-pool summaries store median clean candidate margin and q90 paired cost drift; they do not store the lower-tail margin and upper-tail drift summaries needed to instantiate a calibrated q10/q95 certificate. We therefore call this gap a mechanism proxy, not a planner-margin certificate. Thus this audit instantiates the theorem only at the proxy level supported by the recorded summaries: $K$ is observed from the fixed candidate pool, while the single-candidate tail and failure probabilities are not calibrated. ATR-q90, the cost-margin proxy, and SMPR are empirical counterparts of the radius, planning-margin, and discriminability terms. Because the Gaussian sweep uses equal $\stdmax{}$ increments while the tasks have unequal visual and dynamical sensitivity, aggregate grid-point F1 or interval IoU is not used as the primary validation criterion. Appendix~\ref{sec:appendix-radius-margin-params} instead reports boundary-aware recovery/proxy alignment and the fixed-pool top-1 agreement audit used to interpret this diagnostic.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig_radius_margin_interval_overlay.png}
\caption{Diagnostic validation of the radius--margin proxy against the Gaussian recovery band. Green bands mark behavioral recovery-band points defined only from closed-loop observation-noise scores; blue bands mark positive fixed-pool proxy gaps computed from retained fixed-pool summaries. The right-axis curves show normalized ATR tail and SMPR failure ($1-\mathrm{SMPR}$), both lower-is-better, from the full-sweep diagnostic rows. The shaded diagnostic region is derived from diagnostic statistics only; the behavioral recovery band is shown after the fact for alignment. This is a diagnostic validation, not a new checkpoint-selection algorithm.}
\label{fig:radius-margin-overlay}
\end{figure}
...
The proxy aligns with the broad recovery band but does not exactly reproduce it. TwoRoom and PushT cross the proxy gap within one grid step of the recovery-band onset, Cube aligns on the discrete grid, and Reacher is conservative by two grid steps. This is the expected level of evidence for a diagnostic certificate: it explains the radius--margin mechanism behind the observed recovery band without replacing closed-loop evaluation. The Reacher mismatch is informative rather than hidden: fixed-pool top-1 agreement already rises at the behavioral recovery onset, while the retained-summary cost-margin proxy becomes positive later. To keep the fixed-pool cost-margin proxy separate from a checkpoint-selection claim, we do not turn this overlay into a radius--margin ATR/SMPR gate table. The full-sweep ATR/SMPR held-out validation in \Cref{sec:exp-full-sweep-diagnostics} handles recovered-vs-fragile diagnostic separation; this subsection isolates the retained fixed-pool cost-margin mechanism and its limits.
...
In this interpretation, $\beta_{\mathrm{plan}}$ and $\beta_{\mathrm{disc}}$ name empirical failure components rather than calibrated probabilities. The planning component is represented by the available fixed-pool cost-margin proxy and top-1 agreement audit, and the discriminability component is represented by SMPR, equivalently its failure rate $1-\mathrm{SMPR}$. The table intentionally reports only the retained q90 cost drift and q50 clean margin because the diagnostic summaries do not include lower-tail clean-margin and upper-tail paired-drift fields for a q10/q95 numerical certificate, nor sample-level radii for q95/q99 sensitivity; we therefore do not manufacture certificate-strength tail claims from unavailable raw tails. The closed-loop recovery band explains the fixed-pool radius--margin overlay rather than converting the separate ATR/SMPR diagnostic-region audit into a threshold-classifier claim.

\subsection{Remediation audit tables}\label{sec:appendix-remediation-audits}

The held-out validation in \Cref{tab:heldout-diagnostic-validation} is supported by retained-summary fixed-pool and threshold-sensitivity audits. \Cref{tab:fixed-pool-tail-audit} reports full-sweep top-1 agreement and proxy-gap rows selected from the retained fixed-pool summaries. It does not report pool cert-pass rates because the sample-level maximum cost-drift traces needed for that sufficient event are not retained. \Cref{tab:threshold-quantile-sensitivity} checks that recovered-vs-fragile ATR/SMPR direction is stable under reasonable behavioral-label thresholds; unavailable tail variants are recorded in the audit outputs rather than inferred.
