...
\begin{abstract}
Latent predictive world models avoid pixel reconstruction, but pixel-free prediction is not a robustness definition for closed-loop control. We study this gap with action-conditioned predictive consistency (ACPC), a no-retraining diagnostic for JEPA-style control checkpoints. ACPC compares clean and corrupted histories of the same state after the same action rollout, and pairs this contraction target with a task-grounded near-boundary proxy margin so that action-relevant distinctions are not collapsed. The theory is used as diagnostic calibration: fixed-candidate ACPC bounds cost drift under a margin condition, sampled-pool ACPC stability separates ACPC-tail and clean-margin terms, and a local Gaussian expansion identifies the measured basin radius as action-conditioned encoder--predictor sensitivity. On four control tasks, no-noise LeWM is fragile under observation-only Gaussian noise, while input-side noise training yields recovered robustness plateaus across training seeds 3072/3073/3074. A seed-3072-fixed ACPC/PCC/CRA/MAF rule localizes held-out plateaus (7/8 held-out blocks within 5 points) without dominating simple endpoint baselines. Task-grounded proxy margin pass-rates rise from $0.34$--$0.73$ to $0.99$--$1.00$, and fixed-pool/CEM trace audits show lower clean/noisy planner-side sensitivity at recovered endpoints. Blur/resize checks are bounded scope evidence: diagnostics move with TwoRoom/Reacher improvements and remain weak or mixed when PushT/Cube show no clear score gain. The contribution is a controlled no-retraining diagnostic for JEPA robustness plateaus, not a universal transfer or training-objective claim.
...
\paragraph{C1 --- Control-facing diagnostic principle.} We operationalize visual robustness for JEPA world-model control as a staged diagnostic: encoder geometry shows whether visual noise enters the latent state; ACPC asks whether the shift changes action-conditioned futures and candidate costs; a discriminability countercondition checks that action-, transition-, or cost-distinct states remain separated.

\paragraph{C2 --- Calibration link.} Under a Lipschitz cost readout, bounded rollout disagreement bounds fixed-candidate cost drift. For a once-sampled candidate pool, top-1 instability decomposes into ACPC-tail and clean-margin terms. A local Gaussian linearization then shows that the measured basin radius estimates action-conditioned encoder--predictor sensitivity $\|J_GJ_E\|_F$, not encoder invariance alone. These results justify the readouts as diagnostics while leaving adaptive CEM resampling, repeated replanning, and closed-loop trajectories to empirical tests.

\paragraph{C3 --- Fixed-checkpoint evidence chain.} On fixed checkpoints, we report three-training-seed Gaussian recovery, a selector-baseline-qualified plateau-localization audit, margin-conditioned fixed-pool action flips, a reduced-budget offline CEMSolver trace, task-grounded near-boundary proxy margin pass-rate, and bounded blur/resize scope checks. Larger grids, PLDM replication, ablations, and artifact paths are kept in the appendix and release manifest.
...
The main text keeps three reader-facing diagnostics: closed-loop success/drop for the behavioral effect, the ACPC rollout/cost family ($R_F$, PCC/CRA/MAF, margin-conditioned flips, and a small CEM trace audit) for action-conditioned predictive stability, and the task-grounded near-boundary proxy margin for selective discriminability. Auxiliary ADM/SPRR, latent-noise, artifact-key, and full representation diagnostics remain in the appendices.
...
\Cref{tab:sweep-summary} summarizes the sweep points needed for the main argument; the complete 8-level grid is released in the manifest-listed canonical evaluation artifact. All entries use the unified $n = 3$ seeds (42/43/44) $\times$ 100 trajectories protocol.

\begin{table}[H]
\centering
\caption{LeWM noise-sweep summary. Values are success rate mean $\pm$ population std across evaluation seeds. The two point-best columns may refer to different training-noise levels; they summarize the sweep envelope, while \Cref{fig:sweep} and the manifest-listed canonical evaluation artifact provide the grid context.}
...
The frozen rule is near the closed-loop plateau on held-out seeds without using their closed-loop scores: development seed 3072 gives 3/4 blocks within 5pp with $2.33 \pm 3.49$ pp regret; held-out seeds 3073/3074 give 7/8 blocks within 5pp with $2.21 \pm 1.83$ pp regret and a block-bootstrap 95\% CI of $[1.04, 3.54]$; all seeds give 10/12 blocks within 5pp with $2.25 \pm 2.51$ pp regret. The task-level breakdown below is more useful for reading the failure modes.
...
This comparison localizes where the intervention changed same-state perturbation behavior. The compact rows summarize the high-noise regime visible in closed-loop evaluation; the fixed-rule validation in \Cref{tab:three-seed-diagnostic-validation} audits plateau localization across the full three-seed grid. The full basin grid is released as \codebrk{assets/paper1\_data/acpc\_basin\_diagnostics.json}, and \Cref{sec:exp-downstream-readouts} reports a compact shared-candidate downstream check.
...
\begin{table}[H]
\centering
\caption{Compact LeWM shared-candidate readouts at the primary observation-noise endpoint. Lower is better for ACPC-$H$/transition, PCC, and MAF; higher is better for CRA. The full LeWM/PLDM readout artifact is listed in \code{DATA\_MANIFEST.md}.}
...
Appendix~\ref{sec:appendix-cem-trace} narrows the remaining planner gap with a reduced-budget offline CEMSolver trace on the same three seeds and base/std0.08 endpoints. Under matched clean/noisy histories and the same solver seed, the fixed high-noise endpoint lowers final plan L2 per dimension on all tasks (for example PushT $1.96\to0.64$, Reacher $1.15\to0.38$) and sharply reduces final best-cost deltas. Seeded top-1 flips are also lower on PushT, Reacher, and Cube, but remain high on TwoRoom after CEM updates; this is planner-side evidence for reduced optimizer sensitivity, not a complete CEM stability claim.
...
Low ACPC would be uninformative if it came from collapsed representations. Because 8-step drift already appears in the ACPC-basin section as rollout-space localization, \Cref{tab:diag-base-vs-repr} keeps only the compact representation checks: effective rank, transition-resolution ratio, and inverse-dynamics probe $R^2$. \Cref{tab:semantic-margin-passrate} then adds the task-grounded near-boundary proxy margin readout.

\begin{table}[H]
\centering
\caption{Compact representation discriminability checks: LeWM-base $\to$ the fixed \stdmax{} $=0.08$ noise-trained checkpoint. These checks are paired with the task-grounded near-boundary proxy margin pass-rate in \Cref{tab:semantic-margin-passrate}.}
...
\midrule
TwoRoom & 0.0 & $0.34 \pm 0.05$ & $17.16 \pm 0.60$ & $15.39 \pm 0.11$ & $-2.16 \pm 0.57$ & $0.51$ \\
TwoRoom & 0.08 & $0.99 \pm 0.01$ & $0.77 \pm 0.09$ & $15.80 \pm 0.51$ & $15.27 \pm 0.54$ & $0.51$ \\
PushT & 0.0 & $0.44 \pm 0.17$ & $18.39 \pm 1.24$ & $17.59 \pm 0.21$ & $-0.58 \pm 1.49$ & $1.21$ \\
PushT & 0.08 & $1.00 \pm 0.00$ & $0.70 \pm 0.06$ & $17.54 \pm 0.06$ & $16.27 \pm 0.05$ & $1.21$ \\
Reacher & 0.0 & $0.73 \pm 0.03$ & $15.44 \pm 0.51$ & $18.79 \pm 0.07$ & $2.71 \pm 0.38$ & $1.36$ \\
Reacher & 0.08 & $1.00 \pm 0.00$ & $0.27 \pm 0.02$ & $18.82 \pm 0.08$ & $18.50 \pm 0.06$ & $1.36$ \\
Cube & 0.0 & $0.45 \pm 0.03$ & $19.33 \pm 0.15$ & $19.08 \pm 0.12$ & $-0.27 \pm 0.12$ & $3.90$ \\
Cube & 0.08 & $1.00 \pm 0.00$ & $0.49 \pm 0.05$ & $19.24 \pm 0.06$ & $18.69 \pm 0.13$ & $3.90$ \\
\bottomrule
\end{tabular}
\end{table}

The task-grounded near-boundary proxy margin supplies the missing selective half of ACPC more directly than rank or ID probes alone, while staying below an oracle semantic-label claim. The high-noise endpoint keeps label-crossing proxy-different rollouts separated while shrinking same-state noisy rollouts on all four tasks; the no-noise baselines are weaker under this stricter audit, with pass-rates as low as $0.34$ on TwoRoom and $0.44$ on PushT. Each task/endpoint aggregates three training seeds and 61--100 label-crossing anchors per seed. Treating these local pairs as independent only for calibration, Hoeffding gives $p \ge \hat p-\sqrt{\log(1/\alpha)/(2n)}$; for the smallest high-noise row (TwoRoom, 183 total anchors, $\hat p=0.989$) this gives a one-sided lower calibration near $0.90$, and for 300-anchor rows near $0.93$. Nearby dataset windows may be dependent, so we use this as finite-sample calibration rather than a dataset-wide formal guarantee. The heteroscedastic-loss ablation in Appendix~\ref{sec:appendix-D} remains the failure case that motivates the guard: PushT transition L2 falls from $0.3015$ to $0.1023$, the ID probe falls from $0.7739$ to $0.2678$, and unperturbed success collapses to $13.33\%$. The longer representation, rollout-drift, latent-noise, and task-by-task mechanism diagnostics remain in Appendix~\ref{sec:appendix-diagnostic-framework}--\ref{sec:appendix-mech-attribution}.
...
This is a controlled diagnostic study, not a method paper. The primary behavior statistic is the three-training-seed Gaussian replication (canonical seed 3072 plus independent seeds 3073/3074); evaluation-seed variance is kept as the within-seed measurement scale. These standard deviations are descriptive over three training seeds and three evaluation seeds, not asymptotic significance claims, and correlated diagnostic windows are treated as calibration evidence. Gaussian pixel noise remains the training/evaluation axis. Blur/resize scores, PLDM replication, and failure-case ablations are scope checks. We do not claim superiority over DrQ-style augmentation, TD-MPC2, DreamerV3, or robust MPC methods; the contribution is a no-retraining diagnostic for JEPA latent world-model checkpoints under a controlled Gaussian stressor.

\paragraph{How to use the diagnostics.}
The readouts form an evidence chain rather than a single scalar. Closed-loop evaluation establishes the behavior; $R_F$ and the rollout/cost readouts localize predictive-dynamics change; the margin-conditioned action-flip audit tests the clean-margin side of the sampled-pool link on fixed candidate sets without estimating the full adaptive CEM distribution or environment-feedback loop; the reduced-budget offline CEMSolver trace reports final plan L2 per dimension and cost/rank sensitivity inside the optimizer loop, but is not a complete CEM stability claim; the seed-3072-fixed rule checks whether the same readouts stay near held-out plateaus without replacing closed-loop evaluation; and the task-grounded near-boundary proxy margin pass-rate checks selective contraction without treating it as an oracle hand-labeled semantic proof. Cube remains the useful boundary case, so point-best \stdmax{} values are interpreted as plateau context rather than as a fine-grained ranking target.

\paragraph{What remains.}
The data reject a too-strong reading of latent prediction: future-latent prediction alone does not guarantee closed-loop visual robustness. They also do not prove a universal cross-perturbation robustness claim. Stronger discriminability evidence would require hand-labeled or simulator-derived contact, topology, action-value, or cost-to-go labels, especially for PushT and Cube. The heteroscedastic NLL probe and target-view ablation bound the method story: prediction difficulty alone is the wrong gate, and perturbed-history $\to$ original-future targets are not sufficient here. A paired predictive-dynamics consistency objective remains the separate method-paper path after diagnostic validity is established.

%% ===========================================================================
\section{Conclusion}\label{sec:conc}

This paper frames Gaussian-noise robustness in JEPA latent world-model control as a staged diagnostic problem. Encoder geometry shows how visual perturbations enter the latent state; ACPC asks whether they change action-conditioned futures and candidate costs; and a task-grounded near-boundary proxy margin checks whether robustness is bought by erasing coarse action-relevant distinctions. On fixed LeWM checkpoints, input-side noise training recovers broad matched-Gaussian robustness plateaus across three training seeds, and the paired diagnostics localize that recovery in rollout, candidate-cost, margin, and reduced-budget CEM trace readouts.
...
\paragraph{Hoeffding calibration for the task-grounded near-boundary proxy margin.}
...
\subsection{Full LeWM noise-sweep grid}\label{sec:appendix-lewm-sweeps}
The complete LeWM 4-task $\times$ 9-configuration sweep behind \Cref{tab:sweep-summary,fig:sweep} is released in \codebrk{assets/paper1\_data/canonical\_evals\_20260517.json} and indexed in \code{DATA\_MANIFEST.md}. The PDF keeps the compact sweep summary and figure rather than duplicating the full grid tables.

\subsection{Full LeWM ACPC-basin grid}\label{sec:appendix-acpc-basin-grid}
The full 36-row LeWM ACPC-basin grid is released in \codebrk{assets/paper1\_data/acpc\_basin\_diagnostics.json}; the compact baseline-vs-recovered rows used for the argument are in \Cref{tab:acpc-basin}. This keeps the PDF focused on the evidence needed to read the claim while preserving the full grid as an auditable artifact.
...
\Cref{fig:sweep} is produced by the main plot script listed in \code{DATA\_MANIFEST.md}; no checkpoint or model loading is required.
...
\subsection{Perturbation visualization assets}\label{sec:appendix-A-perturbation-viz}
Representative Gaussian-noise and blur renderings for all four tasks are released under \codebrk{assets/paper1\_figs/corruption/}. The PDF omits these perturbation panels because the perturbation definitions and score tables determine the protocol.
...
The full LeWM-base diagnostic profile is released in \codebrk{assets/paper1\_data/canonical\_diagnostics\_20260517.json}; reader-facing key names are indexed in \code{DATA\_MANIFEST.md}. The main text keeps only the compact discriminability and rollout/cost readouts needed for the argument.
...
\begin{table}[H]
\centering
\caption{Heteroscedastic-loss evaluation (canonical 3-seed $\times$ 100 protocol). The LeWM+noise rows use representative observation+goal 0.08 high-noise rows (TwoRoom \stdmax{} $= 0.08$, PushT \stdmax{} $= 0.06$), matching the strongest stress column of this ablation; the observation-only representative rows in the canonical sweep artifact differ.}
...
The exploratory clean-goal paired ACPC release covers LeWM and PLDM, four tasks, and nine training-noise levels per task. It is released as \codebrk{assets/paper1\_data/acpc\_phase0\_clean\_goal\_seed9101.json}, with the three-seed LeWM extension in \codebrk{assets/paper1\_data/acpc\_phase0\_lewm\_three\_seed.json}. The PDF keeps the compact LeWM readout table in \Cref{tab:acpc-downstream-main}; the full ADM/SPRR/top-$k$ release remains an exploratory artifact and is not used for model selection.
...
\paragraph{Incremental explanatory audit.} A stricter no-retraining audit asks whether the paired readouts add signal beyond the obvious training-noise level. We use the same 96 nonzero LeWM rows, rank-transform each metric/outcome pair, control for $\stdmax{}$, $\stdmax{}^2$, task fixed effects, and training-seed fixed effects, and then report partial $r$, partial $R^2$, incremental $R^2$, and a task--training-seed block permutation test. The released artifact is \codebrk{assets/paper1\_data/selector\_incremental\_audit\_20260704.json}.

\begin{table}[H]
\centering
\caption{Incremental explanatory audit after controlling for training-noise level, task, and training seed. Metrics are oriented so larger is better; reduced drop is the negated clean-to-observation-noise-0.08 drop. The result is modest residual localization evidence, not selector dominance.}
\label{tab:selector-incremental-audit}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lrrrrr}
\toprule
Metric & $r_{\text{drop}}$ & partial $R^2$ & incr. $R^2$ & block $p$ & $r_{\text{obs}}$ \\
\midrule
...
\paragraph{Incremental reading.} The audit further limits the claim. The clearest residual signal is reduced Gaussian drop, with CRA strongest and the aggregate rule modest. Absolute noisy success remains weak after controls. The fixed family is therefore a drop-recovery localization signal and plateau triage aid, not an absolute performance ranker or a superior selector over simple endpoint baselines.

%% ===========================================================================
\section{Reduced-budget offline CEM trace audit}\label{sec:appendix-cem-trace}

The fixed-pool diagnostics deliberately share candidate sets, whereas the deployed planner updates a CEM proposal distribution. To partially close this planner gap without retraining or new closed-loop sweeps, we run the actual \codebrk{stable\_worldmodel.solver.CEMSolver} on logged states from the same base and fixed $\stdmax{}=0.08$ checkpoints across training seeds 3072/3073/3074. This audit uses a reduced CEM budget (4 states per task--seed--endpoint, horizon 5, action block 5, 64 samples, 8 CEM iterations, top-8 elites) and the same solver seed for clean/noisy histories. It is an offline optimizer trace, not a replacement for closed-loop evaluation.

\begin{table}[H]
\centering
\caption{Offline CEMSolver clean/noisy trace over training seeds 3072/3073/3074. Final plan L2/dim and first-action L2/dim compare the final clean/noisy CEM plans after the reduced-budget optimizer run. Seeded top-1 flip uses matched solver sample indices and is meaningful as a trace diagnostic, not as a full adaptive CEM guarantee.}
...
\midrule
TwoRoom & 0.0 & $1.24 \pm 0.14$ & $0.98 \pm 0.09$ & $0.83 \pm 0.12$ & $0.16 \pm 0.04$ \\
TwoRoom & 0.08 & $0.61 \pm 0.13$ & $0.73 \pm 0.24$ & $0.92 \pm 0.12$ & $0.32 \pm 0.09$ \\
PushT & 0.0 & $1.96 \pm 0.04$ & $1.41 \pm 0.13$ & $1.00 \pm 0.00$ & $0.09 \pm 0.01$ \\
PushT & 0.08 & $0.64 \pm 0.03$ & $0.79 \pm 0.13$ & $0.75 \pm 0.20$ & $0.39 \pm 0.10$ \\
Reacher & 0.0 & $1.15 \pm 0.04$ & $1.17 \pm 0.02$ & $0.83 \pm 0.24$ & $0.22 \pm 0.04$ \\
Reacher & 0.08 & $0.38 \pm 0.00$ & $0.52 \pm 0.05$ & $0.25 \pm 0.20$ & $0.53 \pm 0.06$ \\
Cube & 0.0 & $1.30 \pm 0.09$ & $1.04 \pm 0.01$ & $1.00 \pm 0.00$ & $0.12 \pm 0.04$ \\
Cube & 0.08 & $0.51 \pm 0.08$ & $0.50 \pm 0.03$ & $0.42 \pm 0.12$ & $0.38 \pm 0.12$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Reading.} The trace supports the same direction as the fixed-pool diagnostics for plan displacement and best-cost stability: the fixed high-noise endpoint reduces final clean/noisy plan displacement and final best-cost deltas on every task. It does not prove complete adaptive CEM stability: TwoRoom still has high final seeded top-1 flip despite lower plan displacement and higher top-k overlap. We therefore use this audit only to reduce the fixed-pool/planner gap, not to replace closed-loop evaluation or the fixed-candidate theory limits.


%% ===========================================================================
\section{Target-view ablation completion check}\label{sec:appendix-target-view}

This appendix records the completed LeWM target-view ablation referenced in \Cref{sec:disc}. The runs use the origin-future target configuration: the prediction context is encoded from the configured Gaussian-perturbed history, while the prediction target remains the original future latent. This is the perturbed-history $\to$ original-future branch, contrasted with the default full-sequence perturbed-target branch used by the main LeWM+noise sweep. The released summary artifact is listed in \code{DATA\_MANIFEST.md}.
...
\paragraph{Reading.} The ablation acts as a negative scope check and supports retaining the full-sequence empirical branch. Simply changing the prediction target to the original future does not reproduce the recovery from input-side noise augmentation in \Cref{tab:sweep-summary,fig:sweep}: PushT and Reacher remain close to their no-noise robustness cliffs, TwoRoom degrades under stronger eval noise despite high unperturbed success, and Cube does not improve over the main sweep.
