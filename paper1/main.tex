...
\begin{abstract}
Latent predictive world models such as JEPAs avoid pixel reconstruction, but pixel-free prediction is not a robustness definition for closed-loop control. We introduce action-conditioned predictive consistency (ACPC), a no-retraining diagnostic: paired clean/corrupted histories of the same state are rolled out under the same actions, compared through rollout and cost readouts, and checked against a local task-feature margin so that nuisance contraction does not erase action-relevant distinctions. We give a fixed-candidate analysis linking ACPC to cost drift and top-1 stability, a sampled-pool bound separating ACPC-tail and clean-margin terms, and a local Gaussian expansion showing that the ACPC basin measures action-conditioned encoder--predictor sensitivity. On PushT, TwoRoom, Reacher, and Cube, no-noise LeWM is fragile under observation-only Gaussian noise (PushT $86.33\%\to4.33\%$, Reacher $58.67\%\to18.33\%$), while full-sequence input-side noise training supplies recovered checkpoints; the Gaussian recovery pattern holds across training seeds 3072/3073/3074. A seed-3072-fixed ACPC-family rule stays near broad held-out plateaus (7/8 held-out blocks within 5 points) but is not stronger than simple plateau baselines, and a margin-conditioned fixed-pool audit reduces top-quartile noisy top-1 flips to $0.00$ at the fixed high-noise endpoint. The local task-feature proxy margin pass-rate rises from $0.53$--$0.81$ for no-noise baselines to approximately $1.00$ at $\stdmax=0.08$. PLDM replication, negative target-view and heteroscedastic ablations, and blur/resize checks bound the contribution to an auditable matched-Gaussian diagnostic study.
...
\paragraph{C3 --- Three-seed validation and release.} We report four main empirical audits: training-seed robustness, selector-baseline-qualified plateau localization over the full three-seed grid, margin-conditioned fixed-pool action-flip rates, and local task-feature proxy margin pass-rate. Appendices keep the larger LeWM/PLDM grids, blur/resize scope checks, ablations, and implementation key maps out of the main narrative.
