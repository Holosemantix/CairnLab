...
- **Three-training-seed strongest-only unseen score artifacts**: `assets/paper1_data/unseen_origin_vs_std008_strongest_tworoom.json`, `assets/paper1_data/unseen_origin_vs_std008_strongest_reacher.json`, `assets/paper1_data/unseen_origin_vs_std008_strongest_s3073.json`, and `assets/paper1_data/unseen_origin_vs_std008_strongest_s3074.json` (audited score artifacts for the unseen score aggregate over training seeds 3072/3073/3074; together cover 4 tasks × 3 training seeds × 2 std keys × 2 stress families with no-op plus blur k=15 / resize factor 0.25)
- **Unseen Phase-0 ACPC subset artifact**: `assets/paper1_data/unseen_phase0_acpc_subset.json` (matched diagnostic review artifact: 8 selected seed/task/stress case rows for training seeds 3073/3074 joining 16 clean-goal Phase-0 diagnostic rows with strongest-only unseen eval scores; seed 3072 is included in the score aggregate but has no released matched Phase-0 diagnostic rows in this subset)
...
- **Three-seed fixed-rule diagnostic validation**: `assets/paper1_data/three_seed_diagnostic_validation.json` and `.md` (fixed ACPC/PCC/CRA/MAF ranking rule over the 108-row three-seed LeWM full grid; reports 10/12 within-5pp selection hits and 2.25 ± 2.51 pp mean regret to best)
- **Semantic margin pass-rate**: `assets/paper1_data/semantic_margin_passrate_lewm_three_seed.json` and `.md` (24 ok rows: training seeds 3072/3073/3074 × 4 tasks × {0.0, 0.08}; reports task-semantic selective ACPC guard pass rates)
- **Validation remediation summary**: `assets/paper1_data/prospective_validation_summary.json` and `.md` (summarizes three-training-seed Gaussian behavior, full-grid fixed-rule diagnostic validation, semantic margin pass-rate, and bounded unseen-stressor scope checks)
- Scope: 36 LeWM canonical checkpoints = 4 tasks × 9 configs (`base` + `std_max` 0.01..0.08); 36 PLDM checkpoints on the same grid; completed LeWM Gaussian lockbox sweeps for independent training seeds 3073/3074 are summarized separately; strongest-only unseen score artifacts cover training seeds 3072/3073/3074.
...
| `three_seed_diagnostic_validation.json` | Three-seed fixed-rule diagnostic validation | `01c9723e938c718ae484b0d57c4a2fd3ccdbbd4d7799f3499b1702e2757d0e5a` |
| `semantic_margin_passrate_lewm_three_seed.json` | Three-seed semantic margin pass-rate | `b4cea993839043f7a5759fbed9645dc290fc4373b3203aa5ca96442b81a1c3fa` |
| `prospective_validation_summary.json` | Validation remediation summary, three-seed diagnostic validation, semantic pass-rate, and unseen scope check | `bb4baa3ca601e48600ea9b1d3e35d6fafc13afb701e5518f0fdf38ba5d4accca` |
...
- `unseen_phase0_acpc_subset.json` stores the matched unseen diagnostic subset: TwoRoom/Reacher blur as positive-transfer cases and PushT/Cube resize as boundary cases, each comparing `std_key` 0.0 vs 0.08 for training seeds 3073/3074 with clean-goal Phase-0 paired diagnostics. It joins diagnostics with strongest-only unseen eval scores and is a review artifact for matched diagnostic analysis. The score-side unseen aggregate in `prospective_validation_summary.json` separately covers training seeds 3072/3073/3074.
