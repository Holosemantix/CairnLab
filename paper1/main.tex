...
\paragraph{Reading.} The audit evaluates membership enrichment, not point-optimal ranking. The aggregate screen is useful, but not uniquely dominant: MAF-only and the high-std reference are competitive. The high-std reference is a coarse intervention-order screen: larger noise often helps before plateau entry, but once a task--seed block reaches the plateau, \stdmax{} is treated only as a candidate label, not as a monotone performance predictor. We therefore read the screen as diagnostic enrichment and failure localization; closed-loop evaluation remains the authority.
