...
\subsection{Finite-difference Gaussian sensitivity audit}\label{sec:exp-gaussian-sensitivity}

To check the local Gaussian sensitivity interpretation without claiming a global guarantee, we run a finite-difference audit on the base, recovery-onset, and endpoint checkpoints. For each task and training seed, we perturb the same observation histories with small Gaussian noise levels and estimate $\mathbb E[R_\sigma^2]/\sigma^2$ from the action-conditioned rollout radius. \Cref{tab:gaussian-sensitivity-audit} reports a consistent endpoint/base reduction in this local slope, from $0.002$ to $0.008$ of the no-noise-checkpoint value across tasks, alongside the ATR contraction. This supports the composed encoder--predictor sensitivity explanation for the matched Gaussian sweep, while remaining a finite-difference proxy rather than a closed-loop robustness proof.
