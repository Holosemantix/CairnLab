...
- **Residual diagnostic audit**: `assets/paper1_data/residual_diagnostic_audit_20260704.json` and `.md` (derived from the same 96 nonzero LeWM rows; partial Spearman after residualizing on `std_max`, task fixed effects, and training-seed fixed effects, with task-seed block-bootstrap intervals; intended to test residual drop-recovery signal beyond training-noise level rather than selector dominance)
...
- **Local task-feature proxy margin pass-rate**: `assets/paper1_data/semantic_local_margin_lewm_three_seed.json` (24 ok rows: training seeds 3072/3073/3074 × 4 tasks × {0.0, 0.08}; uses the closest 35% state-distance neighborhood and maximum task-feature contrast; reports local proxy pass rates plus same-state noisy radius and local proxy-different rollout distance). The earlier median-distance sanity artifact remains at `assets/paper1_data/semantic_margin_passrate_lewm_three_seed.json` and `.md`.
...
- `residual_diagnostic_audit_20260704.json` stores the no-retraining residual diagnostic audit for the same 96 nonzero LeWM rows. It rank-transforms metric/outcome pairs, residualizes them on `std_max`, task fixed effects, and training-seed fixed effects, and reports task-seed block-bootstrap intervals to separate drop-recovery signal from simple training-noise-level tracking.
