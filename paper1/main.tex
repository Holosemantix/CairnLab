...
Endpoint diagnostics are supplemented by a training-free full-sweep analysis over all $4\times3\times9=108$ task--training-seed--checkpoint rows. \Cref{fig:full-sweep-diagnostics} overlays the closed-loop observation-noise score with normalized ATR tail and SMPR failure ($1-\mathrm{SMPR}$). The recovered portions of the Gaussian sweep consistently occupy lower normalized ATR and lower SMPR failure than fragile rows. This keeps the ACPC evidence focused on recovered-vs-fragile radius/guard separation; fixed-pool top-1 disagreement and proxy-gap traces are kept in Appendix~\ref{sec:appendix-fixed-pool-details}, where they support the planner-side radius--margin audit.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig_full_sweep_diagnostics.png}
\caption{Recovered rows align with lower radius and guard failures across the Gaussian training grid. Each task panel averages training seeds 3072/3073/3074 at the same \stdmax{} value. Green bands mark behavioral recovery-band points; the black curve is the observation-noise score, and the right-axis curves are normalized ATR tail and SMPR failure ($1-\mathrm{SMPR}$), both lower-is-better diagnostics. Planner-side top-1 disagreement is separated into Appendix~\ref{sec:appendix-fixed-pool-details}.}
...
These tables give the numeric values behind \Cref{fig:gaussian-sensitivity-mechanism}. The finite-difference audit and exact-JVP/Hutchinson audit use different local estimators, so their scales need not match; the paper interprets their within-task endpoint/base directions rather than a shared cross-task threshold. \Cref{fig:jvp-trace-decomposition-heatmap} separates the JVP/Hutchinson trace ratios from the alignment coefficient so the main-text figure remains focused on composed local sensitivity.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig_jvp_trace_decomposition_heatmap.png}
\caption{JVP/Hutchinson decomposition behind the local sensitivity audit. Trace ratios show encoder, rollout, and composed endpoint/base sensitivity estimates; the alignment coefficient is displayed separately because it is not the same scale as the trace quantities. These local diagnostics explain where composed sensitivity changes, but they do not provide a global or closed-loop guarantee.}
