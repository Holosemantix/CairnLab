...
In this interpretation, $\beta_{\mathrm{plan}}$ and $\beta_{\mathrm{disc}}$ name empirical failure components rather than calibrated probabilities. The planning component is represented by the available fixed-pool cost-margin proxy and top-1 agreement audit, and the discriminability component is represented by SMPR, equivalently its failure rate $1-\mathrm{SMPR}$. The table intentionally reports only q90 cost drift because the retained diagnostic summaries do not include q95/q99 paired-drift tails or sample-level radii; we therefore do not manufacture a q90/q95/q99 sensitivity table from unavailable raw tails. The closed-loop recovery band explains the fixed-pool radius--margin overlay rather than converting the separate ATR/SMPR diagnostic-region audit into a threshold-classifier claim.
