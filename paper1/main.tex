...
\caption{Noise-trained endpoints contract the ATR radius tail and improve the SMPR guard on all four tasks. ATR is the q90 same-state clean/noisy action-conditioned rollout disagreement normalized by the clean transition scale; SMPR is the task-grounded near-boundary selective-margin pass rate. Points report means across training seeds 3072/3073/3074 and horizontal bars report population standard deviations. Exact values are in Appendix~\ref{sec:appendix-supplementary-diagnostics}.}
...
Across all $4\times3\times9=108$ task--training-seed--checkpoint rows, recovered portions of the Gaussian sweep consistently occupy lower normalized ATR and lower SMPR failure than fragile rows. \Cref{fig:full-sweep-diagnostics} places the closed-loop observation-noise score above the two diagnostic-failure traces so behavioral recovery and diagnostic movement remain visually distinct. Fixed-pool top-1 disagreement and proxy-gap traces are kept in Appendix~\ref{sec:appendix-fixed-pool-details}, where they support the planner-side radius--margin audit.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{fig_full_sweep_diagnostics.png}
\caption{Recovered rows align with lower radius and guard failures across the Gaussian training grid. Each task block averages training seeds 3072/3073/3074 at the same \stdmax{} value and separates the observation-noise score (upper axis) from normalized ATR tail and SMPR failure ($1-\mathrm{SMPR}$; lower axes). Continuous green bands mark training-noise ranges where a majority of seed rows meet the behavioral recovery criterion. Planner-side top-1 disagreement is separated into Appendix~\ref{sec:appendix-fixed-pool-details}.}
