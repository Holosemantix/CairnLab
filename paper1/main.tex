...
\paragraph{C3 --- Three-seed validation and release.} We report three main empirical audits: training-seed robustness, selector-baseline-qualified plateau localization over the full three-seed grid, and task-state proxy margin pass-rate. Appendices keep the larger LeWM/PLDM grids, blur/resize scope checks, ablations, and implementation key maps out of the main narrative.
...
The diagnostics are used as a ranked evidence chain, not as a single scalar. Closed-loop evaluation establishes the behavior; $R_F$ and the rollout/cost readouts localize the predictive-dynamics change; the seed-3072-fixed rule audits whether the same readouts stay near held-out plateaus without replacing closed-loop evaluation; and the task-state proxy margin checks whether contraction preserves coarse action-relevant distinctions. Cube remains the useful boundary case, so point-best \stdmax{} values are interpreted as plateau context rather than as a fine-grained ranking target.
