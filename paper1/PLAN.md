...
> Last updated: 2026-06-02（merged dense Gaussian-noise ACPC basin diagnostic with Phase 0 exploratory ACPC appendix）。
...
**状态：closer, still not submit-ready；ACPC dense Gaussian-noise basin diagnostic 与 Phase 0 exploratory appendix 已补，仍需 final source audit / build checks。**

- 框架已 reframe：title / abstract / intro / related work / §3 ACPC 概念+诊断 / discussion / conclusion 已围绕 action-conditioned predictive consistency 重写；新增 §4.x Gaussian-noise ACPC basin table。
...
- related work 已明确 ViGMO / Bisim-JEPA / LeJEPA theory 的边界。提交前仍需 references final source audit、consistency checker、LaTeX clean build。
