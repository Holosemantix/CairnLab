...
\begin{abstract}
Latent predictive world models avoid pixel reconstruction, but pixel-free prediction is not a robustness definition for closed-loop control. We study this gap with action-conditioned predictive consistency (ACPC), a no-retraining diagnostic for JEPA-style control checkpoints. ACPC asks whether clean and corrupted views of the same state produce consistent future predictions under the same action rollout, while task-grounded near-boundary pairs test that action-relevant distinctions remain separated. A formal diagnostic link connects these readouts to fixed-candidate cost drift, margin failures, and local encoder--predictor sensitivity; it is not a closed-loop guarantee. Across three independent training seeds and four control tasks, no-noise LeWM is fragile under observation-only Gaussian noise, while input-side noise training yields broad matched-Gaussian robustness plateaus. A rule fixed on one development seed localizes held-out plateaus, but behaves as a triage signal rather than a superior checkpoint selector. Margin and planner-trace audits support the selective interpretation: recovered checkpoints contract same-state noisy rollouts without coarse collapse and reduce clean/noisy optimizer sensitivity. Blur/resize checks remain bounded scope evidence: diagnostics track clear TwoRoom/Reacher gains and stay weak or mixed when PushT/Cube show no clear gain. The contribution is a controlled no-retraining diagnostic for JEPA robustness plateaus, not a universal transfer or training-objective claim.
...
\paragraph{C3 --- Fixed-checkpoint evidence chain.} On fixed checkpoints, we report a three-training-seed evidence chain: matched-Gaussian recovery; a fixed-rule plateau audit qualified by selector baselines and plateau-aware checks; selective-margin and planner-side checks; and bounded blur/resize scope checks. Larger grids, PLDM replication, ablations, and artifact paths remain in the appendix and release manifest.
...
The fixed rule is not a replacement for closed-loop evaluation, and its purpose is narrower than checkpoint selection: it tests whether ACPC-family readouts localize broad plateaus on held-out seeds. Because the sweep has broad finite-variance plateaus, Appendix~\ref{sec:appendix-selector-audit} treats \stdmax{} as a candidate label inside each task--training-seed block rather than as a continuous variable to be predicted pointwise. With a 5pp plateau tolerance, the aggregate rule gives 10/12 plateau hits, $0.33 \pm 0.92$ pp regret-to-plateau, and 88.3\% concordance on 141 decisive checkpoint pairs whose closed-loop scores differ by more than 5pp. This supports plateau triage, not selector dominance: fixed high-noise and MAF-only baselines are comparable or stronger on the same audit. The held-out audit is strongest on TwoRoom and Reacher, useful but imperfect on PushT, and weakest on Cube. The clearest miss is Cube seed 3072: the fixed rule chooses $\stdmax{}=0.05$ with $60.00\%$ observation-noise 0.08 success, while the closed-loop best is $\stdmax{}=0.03$ at $68.33\%$ (regret $8.33$ pp). PushT seed 3073 is the other beyond-5pp miss ($81.00\%$ selected at $\stdmax{}=0.08$ versus $86.67\%$ best at $\stdmax{}=0.04$). A selector-baseline audit in Appendix~\ref{sec:appendix-selector-audit} shows the same boundary: the aggregate rule is comparable to fixed $\stdmax{}=0.08$ and single-metric selectors on broad Gaussian plateaus, but better than exact random nonzero-std selection. We therefore use the fixed rule to explain plateau structure and failure cases, while closed-loop scores remain the authority for final claims.
...
This audit compares the seed-3072-fixed aggregate ACPC/PCC/CRA/MAF rule against simple selector baselines over the same three-training-seed Gaussian grid. All rows use nonzero training-noise candidates only. The random baseline is the exact expected regret over the eight nonzero grid rows, not a Monte Carlo sample. The released audit artifacts are \codebrk{assets/paper1\_data/selector\_baseline\_audit\_20260704.json} and \codebrk{assets/paper1\_data/selector\_plateau\_audit\_20260704.json}.
...
\paragraph{Plateau-aware audit.} The main selector question is not whether the diagnostics predict the point-best \stdmax{} inside a noisy plateau. We therefore add a block-wise audit that treats \stdmax{} as a candidate label inside each task--training-seed block. A 5pp closed-loop tolerance defines the practical plateau, and decisive-pair concordance ignores checkpoint pairs whose observation-noise 0.08 scores differ by at most 5pp. The released artifact is \codebrk{assets/paper1\_data/selector\_plateau\_audit\_20260704.json}.

\begin{table}[H]
\centering
\caption{Plateau-aware selector audit. Regret-to-plateau is $\max(0,\text{best}-\text{selected}-5\text{pp})$. Decisive-pair accuracy ranks only checkpoint pairs separated by more than 5pp in closed-loop observation-noise 0.08 success. The fixed/high-std row selects fixed $\stdmax{}=0.08$ for plateau hit and uses monotone high-std ranking for decisive pairs.}
\label{tab:selector-plateau-audit}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{lcccc>{\raggedright\arraybackslash}X}
\toprule
Rule & plateau hit & bad picks & regret-to-plateau & decisive pairs & reading \\
\midrule
Aggregate ACPC/PCC/CRA/MAF & $10/12$ & $2/12$ & $0.33 \pm 0.92$ & $88.3\%$ & useful triage, not dominance \\
Fixed/high-std baseline & $10/12$ & $2/12$ & $0.17 \pm 0.40$ & $92.9\%$ & competitive simple baseline \\
MAF only & $10/12$ & $2/12$ & $0.44 \pm 1.29$ & $92.6\%$ & comparable single readout \\
Random nonzero std & $8.5/12$ & $3.5/12$ & $4.33 \pm 10.67$ & $50.0\%$ & exact/random reference \\
\bottomrule
\end{tabularx}
\end{table}
