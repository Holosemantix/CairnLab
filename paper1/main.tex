...
\]
A positive proxy gap is the available empirical counterpart of the fixed-pool margin condition. Because the recorded summaries contain median clean candidate margin and q90 paired cost drift rather than the q10/q95 theorem quantities, we call it a fixed-pool proxy and not a planner-margin certificate. Appendix~\ref{sec:appendix-radius-margin-params} reports the fixed-pool candidate count and the plateau/proxy ranges used to interpret this diagnostic.
...
In this interpretation, $\beta_{\mathrm{plan}}$ is the empirical chance that the clean top-1/top-2 fixed-pool margin is too small relative to the cost drift, and $\beta_{\mathrm{disc}}$ is the chance that task-grounded different-state separations fall inside the perturbation tube. The experiments estimate the planning side with available fixed-pool margin summaries and estimate the discriminability side with SMPR. The table intentionally reports only the cost-margin proxy interval and the closed-loop behavioral plateau interval, because it explains the fixed-pool radius--margin overlay rather than converting the separate ATR/SMPR diagnostic-region audit into a threshold-classifier claim.
