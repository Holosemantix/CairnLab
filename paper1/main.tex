...
The evidence shows broad matched-Gaussian recovery plateaus rather than a stable universal point optimum. Input-side noise training recovers the observation-noise endpoint on TwoRoom, PushT, and Reacher and gives a weaker but positive Cube signal across training seeds. At recovered endpoints, ACPC Tail Risk (ATR) is substantially lower and Selective Margin Pass Rate (SMPR) is near one across tasks. Bounded blur/resize checks delimit perturbation scope.
...
\begin{definition}[Matched-perturbation diagnostic interval]\label{def:robust-interval}
...
\]
The matched-perturbation diagnostic interval is the set of checkpoint indices $\rho$ for which $\widehat{\mathcal G}_{\rho,\sigma}$ holds. If planner-margin traces are unavailable, $\widehat r_{\mathrm{margin}}$ must be reported as a task-grounded rollout-space proxy, not as a planner-margin certificate. If diagnostics are available only at the no-noise and recovered endpoints, the evidence is endpoint mechanism evidence rather than interval identification.
\end{definition}
...
The same linearization gives the local tail interpretation used by ATR. Let
\[
A_{h,\mathbf a}=J_{G_{\mathbf a}}(E_\theta(o))J_E(o).
\]
Then $R_\sigma^2(h,\mathbf a)\approx \|A_{h,\mathbf a}\xi\|_2^2$ with $\xi\sim\mathcal N(0,\sigma^2 I)$. If $A_{h,\mathbf a}A_{h,\mathbf a}^\top$ has eigenvalues $\lambda_i$, then
\[
...
\]
Combining this local radius quantile with the fixed-pool margin condition gives the diagnostic noise scale
\[
...
\]
This is a local diagnostic approximation, not a global robustness theorem. It clarifies why matched Gaussian training can lower ATR: it may reduce encoder-side neighborhood crossing, rollout-side amplification, or both.
...
The proxy aligns with the broad recovery interval but does not exactly reproduce it. TwoRoom and PushT cross the proxy gap near the start of their high-score intervals, Cube aligns over the same broad range, and Reacher shows early behavioral recovery before the cost-margin proxy becomes positive. This is the expected level of evidence for a diagnostic certificate: it explains the radius--margin mechanism behind the existing plateau without replacing closed-loop evaluation. To keep the fixed-pool cost-margin proxy separate from a checkpoint-selection claim, we do not turn this overlay into a radius--margin ATR/SMPR gate table. As a separate no-retraining diagnostic-region audit, we group each task--training-seed sweep by normalized closed-loop recovery instead of by a universal \stdmax{} value or a threshold-classifier score. On held-out seeds 3073/3074, fragile checkpoints remain at high normalized ATR (median 1.00) and lower SMPR (median 0.43), transition checkpoints lie between the endpoints, and robust checkpoints occupy a shared low-ATR/high-SMPR region (median normalized ATR 0.087, median SMPR 1.00). Across all eight held-out task--seed blocks, the robust median has lower normalized ATR and higher SMPR than the fragile median. This supports ATR/SMPR as a task-conditioned diagnostic region for robust-vs-fragile separation and direction consistency; closed-loop evaluation remains the behavioral authority.
...
Cube & 65 & $0.03$--$0.08$ & $0.03$--$0.08$ & proxy and plateau ranges align \\
\bottomrule
\end{tabular}
\end{table}

In this interpretation, $\beta_{\mathrm{plan}}$ and $\beta_{\mathrm{disc}}$ name empirical failure components rather than calibrated probabilities. The planning component is represented by the available fixed-pool cost-margin proxy, and the discriminability component is represented by SMPR, equivalently its failure rate $1-\mathrm{SMPR}$. The table intentionally reports only the cost-margin proxy interval and the closed-loop behavioral plateau interval, because it explains the fixed-pool radius--margin overlay rather than converting the separate ATR/SMPR diagnostic-region audit into a threshold-classifier claim.
