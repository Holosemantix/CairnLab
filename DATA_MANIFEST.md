...
- **Legacy residual diagnostic audit**: `assets/paper1_data/residual_diagnostic_audit_20260704.json` and `.md` (derived from the same 96 nonzero LeWM rows; retained for provenance after the stricter incremental audit below replaced it in the paper narrative)
- **Selector incremental explanatory audit**: `assets/paper1_data/selector_incremental_audit_20260704.json` and `.md` (same 96 nonzero LeWM rows; rank-transformed partial regressions controlling for `std_max`, `std_max^2`, task fixed effects, and training-seed fixed effects, plus task-seed block permutation; intended to test whether ACPC-family readouts add modest signal beyond noise-level tracking)
...
| `selector_incremental_audit_20260704.json` | Incremental selector/readout audit with std/task/seed controls | `b9c3c5140746b84c45323329019caee02f2fa1a962d301fe2908dc5b2d003645` |
| `selector_incremental_audit_20260704.md` | Human-readable incremental selector/readout audit summary | `e8d18f895e95d8e4f8824bdb50c0f552f862a1123a783b53d2f938553ff44f71` |
...
- `residual_diagnostic_audit_20260704.json` stores a legacy no-retraining residual diagnostic audit for the same 96 nonzero LeWM rows. `selector_incremental_audit_20260704.json` is the paper-facing stricter audit: it rank-transforms metric/outcome pairs, controls for `std_max`, `std_max^2`, task fixed effects, and training-seed fixed effects, and uses task-seed block permutation to separate modest drop-recovery signal from simple training-noise-level tracking.
